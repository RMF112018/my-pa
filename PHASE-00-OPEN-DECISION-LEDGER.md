...
States: `OPEN_OPERATOR`, `OPEN_REVIEW`, `DEFERRED_PHASE_GATE`, `RECOMMENDED_DEFAULT_ACTIVE_IN_PACKAGE`, `RESOLVED`, `SUPERSEDED`, and `SUPERSEDED_PENDING_OPERATOR_CONFIRMATION`. The last three were added when repository truth answered a decision the package could only defer; the original entry is left visible beside its resolution rather than rewritten. The last of them marks a decision that merged work has already settled in fact but that remains operator-only, so no agent closes it.
...
| `P00-OD-008` | Physical PostgreSQL target? | Disposable isolated DB only; physical alias/connection unresolved and fail-closed | Before existing DB access | Yes | Superseded in fact by the merged migration; one clause narrowed | `SUPERSEDED_PENDING_OPERATOR_CONFIRMATION` |
