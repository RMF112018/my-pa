...
- `domain/identity` — capability, purpose, principal, and operation binding, including all twenty capability names and their operator-only flags. WP-6 added four `capture.*` names and two capture purposes, WP-7 added `capture.search`, WP-8 added `review.list` and `review.decide` with `review_disposition`, WP-9 added `knowledge.reveal` under the existing `capture_review` purpose, WP-11 added `continuity.pulse`, `continuity.situations` and `continuity.projects` under that same purpose, and WP-23 added `knowledge.coverage` under the existing `status_observation` purpose — no purpose was widened, and `domain/identity/operation.py` records what each reuse does and does not cover, including the one residual `knowledge.coverage` carries: a status grant now reaches a corpus-wide row set. None is operator-only because none grants external authority (`D-70`, `D-91`, `D-101`).
...
- `application` — the twenty capability use cases behind one entry point, one shared authorization and disclosure path, and the capability manifest and readiness report derived from that wiring rather than restated. It reaches persistence and providers only through the ports in `contracts/ports`.
- `adapters/http` and `apps/gateway.py run` — the HTTP transport and its composition root. All twenty capabilities are reachable at `POST /v1/<capability>` on `127.0.0.1`, and the response body is the envelope the application produced. Starlette and uvicorn, not FastAPI, and there is no option to bind anywhere but loopback. Two credential planes reach it and they are disjoint: `Authorization: Bearer <token>` in `entra` mode, and `Authorization: ClientCredential <client_id>:<secret>` at `POST /remote/v1/capture.create`, the authenticated remote capture ingress (WP-10). The ingress is **off unless `MY_PA_REMOTE_INGRESS_ENABLED` says otherwise**, its route is mounted either way and refuses `501` in the handler when it is off, and turning it on publishes nothing — the host is still a loopback constant, and terminating TLS or exposing the address is an operator act this repository does not perform. [`ops/runbooks/gateway-operations.md`](ops/runbooks/gateway-operations.md) covers running it.
- `adapters/mcp` and `apps/gateway.py mcp` — the same twenty capabilities over the Model Context Protocol, using the official `mcp` SDK on **stdio only**. The tool list is derived from the capability set and each tool's schema from the command it builds, so nothing about a capability is written down twice. No socket is opened and no credential is read; the SDK's network transports are never imported.
- `apps/cli/clients.py` — the operator command that mints, lists, revokes, and reports remote capture client credentials. Minting a credential is reserved to the operator by `AGENTS.md` section 8.2, so it is a shell command rather than a capability: it builds no `ApplicationService`, names no `Capability`, and prints the credential exactly once because nothing stores one.
...
- Twenty-eight Alembic revisions covering target schemas and extensions, tables, indexes, foreign keys, the migration control plane, views, the `knowledge` schema, extraction, audit, enrollment, capture, proposal, review/promotion, relationship identity, the native-source control plane, the identity plane (user accounts and scope grants), per-Principal capture partitioning, per-Principal review/promotion partitioning, the narrowed extraction status vocabulary, per-Principal partitioning of the two job queues, the audited capability vocabulary widened for `knowledge.reveal`, the registered remote capture client with the transport and trust vocabularies widened for it, the continuity objects (commitments, decisions, tasks), their append-only lifecycle/closure evidence, the Pulse reason-code and evidentiary-basis columns and the audited capability vocabulary widened for the three `continuity.*` capabilities, and the audited capability vocabulary widened again for `knowledge.coverage`, and the managed-document plane — documents, immutable versions, submissions, receipts and append-only lifecycle rows, with a `BEFORE UPDATE OR DELETE` trigger on the two append-only tables; head `4c7b2e91d8a5`. Applied and rolled back in the database tier; only SQL generation is checked by FAST. **No revision derives a closed-set constraint from a domain enum** (`D-69`): historical vocabulary is frozen in each emitting revision and widened by an explicit `ALTER` in the revision that widens it, so an already-merged revision goes on emitting the DDL it emitted on the day it merged.
...
- *"user-authored capture … exists beyond a scaffold README"* was in the
  ...
  not build. **Two of those three have since been built and the sentence is
  corrected rather than left standing**: WP-10 adds the remote transport and the
  registered capture client, minted by `apps/cli/clients.py` and stored as a
  SHA-256 digest, with the submission recording `transport = remote_client` in
  the same durable transaction a local capture writes. There is still no
  delivery-attempt log. Captures carry no owner-scoped access control (`D-72`, and
