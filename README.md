...
in which 53 of the 65 capabilities are `available` and 12 — the six `documents.`
and six `entities.` names — are `not_implemented`, and readiness is `degraded`
with the limitation `12 of 65 capabilities are unwired.` Derived by building the
...
fully composed and default-composition figures in these paragraphs to the
dispatch table and its two explicitly withheld families.
