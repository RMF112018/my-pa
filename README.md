...
`MY_PA_MANAGED_DOCUMENT_ROOT`, `MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED` and
...
in which 53 of the 73 capabilities are `available` and 20 — the six `documents.`
names, the six `entities.` names and the eight `relationship_memory.` names — are
`not_implemented`, and readiness is `degraded` with the limitation
`20 of 73 capabilities are unwired.` Derived by building the manifest both ways
