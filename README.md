...
- `domain/identity` — capability, purpose, principal, and operation binding, including all ninety-nine capability names and their operator-only flags. WP-6 added four `capture.*` names and two capture purposes, WP-7 added `capture.search`, WP-8 added `review.list` and `review.decide` with `review_disposition`, and WP-29 added the nine `relationship_memory.` names with the `relationship_memory_read` and `relationship_memory_authoring` purposes; none is operator-only because none grants external authority (`D-70`, `D-91`, `D-101`).
...
true only of a process given all three variables above. None is set by default
...
in which 53 of the 99 capabilities are `available` and 46 — the six `documents.`
...
`46 of 99 capabilities are unwired.` Derived by building the manifest both ways
