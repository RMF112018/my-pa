...
`capabilities.get` reports the 166 implemented capabilities `available`, the six
new Run 01 names `not_implemented`, and readiness `degraded`, while PDF still
...
in which 101 of the 172 capabilities are `available` and 71 — the six `documents.`
names, the fifty-five `entities.` names, the nine `relationship_memory.` names,
and the one remaining Run 01 name — are
`not_implemented`, and readiness is `degraded` with the limitation
`71 of 172 capabilities are unwired.` Derived by building the manifest both ways
