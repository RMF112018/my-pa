...
in which 55 of the 121 capabilities are `available` and 66 — the six `documents.`
names, the fifty-one `entities.` names and the nine `relationship_memory.` names — are
`not_implemented`, and readiness is `degraded` with the limitation
`66 of 121 capabilities are unwired.` Derived by building the manifest both ways
