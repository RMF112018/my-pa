...
- `domain/identity` — capability, purpose, principal, and operation binding, including all nineteen capability names and their operator-only flags. WP-6 added four `capture.*` names and two capture purposes, WP-7 added `capture.search`, WP-8 added `review.list` and `review.decide` with `review_disposition`, and WP-9 added `knowledge.reveal` under the existing `capture_review` purpose; none is operator-only because none grants external authority (`D-70`, `D-91`, `D-101`).
