...
in which 56 of the 98 capabilities are `available` and 42 — the six `documents.`
names, the twenty-eight `entities.` names and the eight `relationship_memory.` names — are
`not_implemented`, and readiness is `degraded` with the limitation
`42 of 98 capabilities are unwired.` Derived by building the manifest both ways
