...
`capabilities.get` reports all 172 implemented capabilities `available` and
readiness `ready`, while PDF still
reports `decision_gated` pending
`P00-OD-003`. `PC-CM-RUN01-WP07` wired `constraints.create_published`, the last
declared name without a handler, so a fully composed process now reports no
unimplemented capability at all -- which is why this paragraph no longer names
that availability or a degraded readiness. Both figures are derived from the application's own wiring rather
than from a constant, and the reported readiness is a statement about the
application and not about a deployment: a process serves it over HTTP on loopback, and the slice now
...
in which 102 of the 172 capabilities are `available` and 70 — the six `documents.`
names, the fifty-five `entities.` names and the nine `relationship_memory.`
names, which is now the whole of the withheld figure — are
`not_implemented`, and readiness is `degraded` with the limitation
`70 of 172 capabilities are unwired.` Derived by building the manifest both ways
