...
in which 55 of the 109 capabilities are `available` and 54 — the six `documents.`
names, the thirty-nine `entities.` names and the nine `relationship_memory.` names — are
`not_implemented`, and readiness is `degraded` with the limitation
`54 of 109 capabilities are unwired.` Derived by building the manifest both ways
