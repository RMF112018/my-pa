...
- Fifteen Alembic revisions covering target schemas and extensions, tables, indexes, foreign keys, the migration control plane, views, the `knowledge` schema, the extraction tables, the audit events table, the enrolled object set, the capture tables, the proposal tables with the exact capture-search index, the review, promotion, context-link and conversation tables, relationship identity and profiles, and the narrowing of the extraction status vocabulary; head `9d4e7a3b1c62`. Applied and rolled back in the database tier, run green against a disposable PostgreSQL 17.10 instance on 2026-08-09 (`D-109` records the head and the selection); only SQL generation is checked by FAST. **Every revision returns the database to what the one below it denotes**, checked against a server by [`tests/schema/test_every_revision_denotes_one_schema.py`](tests/schema/test_every_revision_denotes_one_schema.py): for each revision, the constraints a freshly built database holds at its parent and the constraints one holds after going up a revision and back down must be identical. The kinds of fact compared, enumerated rather than counted: constraints, columns with type, nullability and default, indexes, triggers, relations, schemas and extensions — nothing enumerated within them. Outside them, and outside every test in this tree: comments, privileges, collations, sequence parameters, enum labels, column order and rows. Six earlier attempts at this claim were each broken by review, which is why it is executed rather than parsed and why the exclusions are listed rather than implied (`D-109`). **No revision derives a closed-set constraint from a domain enum** (`D-69`): the audit vocabulary is a frozen literal in each revision and is widened by an explicit `ALTER` in the revision that widens it, so an already-merged revision goes on emitting the DDL it emitted on the day it merged. `9d4e7a3b1c62` is the same mechanism applied to `extractions.status`, in the narrowing direction (`D-108`). That rule is mechanized only for the closed sets a revision emits through the shared declaration; one written as raw SQL is outside the guard's reach entirely. The revisions that write one, named rather than counted, are `1e6c0a94f3b7`, `2f7d1ba05c48`, `1a4c9e77b2d5`, `2b7e9f4c1a83`, `3c8f1e2a5b74`, `7f2a9d6c4e18` and `9d4e7a3b1c62`. The ones a reader would act on are those whose unseen vocabulary is exactly equal to a live domain closed set, which is the signature the guard exists to find: `7f2a9d6c4e18` (`ResolutionAction`, `EvidenceAuthority`, `personal_fixture._ALLOWED_DOMAINS`) and `3c8f1e2a5b74` (`Capability`, `Purpose`). Read by hand each writes literals and imports no enum, so all of them comply with `D-69` — what is missing is verification, not compliance, and closing that gap needs a rule that reads emitted SQL text rather than the object graph (`D-109`).
