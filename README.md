...
in which 55 of the 115 capabilities are `available` and 60 — the six `documents.`
names, the forty-five `entities.` names and the nine `relationship_memory.` names — are
`not_implemented`, and readiness is `degraded` with the limitation
`60 of 115 capabilities are unwired.` Derived by building the manifest both ways
