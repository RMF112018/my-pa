...
in which 55 of the 118 capabilities are `available` and 63 — the six `documents.`
names, the forty-eight `entities.` names and the nine `relationship_memory.` names — are
`not_implemented`, and readiness is `degraded` with the limitation
`63 of 118 capabilities are unwired.` Derived by building the manifest both ways
