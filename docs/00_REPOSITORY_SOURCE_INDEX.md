...
- [`docs/testing/relationship-intelligence-final-completion.md`](testing/relationship-intelligence-final-completion.md) — campaign `MYPA-RI-FINAL-COMPLETION-CAMPAIGN-20260828-001` work-package, capability, acceptance, validation, and operator-only-action traceability.
...
Running the local candidate on one machine. A procedure under `ops/runbooks/` is normally written only after it has been executed. An explicitly commissioned pre-execution procedure must label itself unexecuted, as the RI final-completion WP-08/WP-09 procedures do; deployment, production activation, and destructive data operations remain operator-gated (`AGENTS.md` section 5).
