...
| 042 | No raw identifier in the primary UX | WP-TUX-05 | E2E + unit | `web/e2e/work-acceptance.spec.ts:178`, `web/e2e/work-acceptance.spec.ts:296`; `web/src/components/work/task-list-row.test.tsx:205`; `web/src/components/pulse/today-task-card.test.tsx:284`; `web/src/components/tasks/use-task-operations.test.tsx:596` | PASS | Diagnostics deliberately retain the identifier; `web/e2e/work-acceptance.spec.ts:178` asserts both halves. **Correction.** `web/e2e/work-acceptance.spec.ts:296` was briefly removed from this row on the stated grounds that it proved only TASK-AC-043's criterion. That was wrong: its forbidden-token list is `in_progress, lifecycle, p1, p2, p3, p4, tsk_`, and `tsk_` is a raw identifier — this row's criterion — asserted on the list row. It is restored, because it is this row's only browser-level *list-row* identifier proof. `web/e2e/work-acceptance.spec.ts:178` proves the same criterion on the compact sheet against the fuller prefix set (`tsk_`, `cap_`, `rdec_`, `tsh_`) and adds the diagnostics-retain half. The row legitimately shares `web/e2e/work-acceptance.spec.ts:296` with TASK-AC-043: one test forbids both vocabulary and identifiers. |
