...
The `my_pa` application defines and wires one hundred and twenty-seven capabilities through HTTP,
MCP, and CLI composition, with PostgreSQL Principal partitioning and two worker
planes. **A default composition exposes fifty-eight of them.** The six
`documents.` names require `MY_PA_MANAGED_DOCUMENT_ROOT`, the
fifty-four `entities.` names require `MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED`
