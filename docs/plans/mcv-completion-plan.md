...
12. **WP-10 — PWA capture surface and offline recovery.** **Deferred until after
    ...
    remains the only frontend package here and retains all of its existing gates.
13. **WP-11 — Native Apple Reminders execution projection.** Internal sequence
    ...
    Conditional on `D-39`, dependent on WP-10, and nothing in it is built. Because
    WP-10 is now post-MCV, WP-11 cannot execute inside the pre-completion sequence
    and joins the disclosed deferred set; `D-104` does not answer any of its nine
    operator decisions or authorize any part of it.
...
| D-08 | Terminal disposition cannot be reached in this scope | Plan §11 requires GoodNotes, frontend, and relationship acceptance criteria that D-03 and D-04 defer. The honest terminal state is MCV-complete with the deferred set named, not `MYPA_CURRENT_PRODUCT_SCOPE_COMPLETE`. **Updated 2026-08-04:** the operator's `D-104` deferral makes that honest terminal state reachable after WP-9. It does not make the deferred work complete or declare the broader current product scope complete. | Reached after WP-9, with deferred set disclosed; formal MCV completion declaration remains an operator act |
...
| D-104 | **WP-10 is deferred until MCV completion; WP-11 follows its dependency into the disclosed deferred set** | Direct operator instruction on 2026-08-04: “defer wp-10 until mcv is complete.” This explicitly overturns `D-32`'s assumed placement of WP-10 inside the pre-completion sequence without lifting the frontend hold or deciding `O-20`. WP-11's published sequence and this plan both make WP-10 its prerequisite, so WP-11 cannot honestly run before completion while WP-10 is deferred; it is deferred with that dependency rather than silently reordered. All `NAR-OP-001`–`NAR-OP-009` gates remain open. WP-0R through WP-9 are merged, making **MCV-complete with WP-10, WP-11, and the previously named deferred set disclosed** the honest implementation terminal state described by `D-08`; this does not assert `MYPA_CURRENT_PRODUCT_SCOPE_COMPLETE`, deploy anything, or convert the still-required formal operator completion declaration into a repository drafting act. | Operator-directed; pre-completion implementation sequence ends at WP-9 |
...
1. **The MCV completion declaration.** *Still reserved to the operator.* `AGENTS.md` section 1
   ...
   repository can now report **MCV-complete with the deferred set named**, the
   terminal state `D-08` defined, but only the operator can issue the formal
   completion declaration. No date is inferred and no declaration is drafted on
