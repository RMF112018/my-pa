...
| Alembic head | `1a4c9e77b2d5` in the repository, eleven revisions; the canonical database remains at `6c4d3ea82f10` because every revision above it builds the `knowledge` schema, which runs only against disposable databases | `migrations/versions/*.py`, `select * from alembic_version` |
...
Ninety-nine Python modules under `src/my_pa` and eighty-eight test modules —
