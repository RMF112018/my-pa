...
One hundred and six Python modules under `src/my_pa` and ninety-six test modules —
...
- `QC-AC-050` — spec `20:221`: "**Exact** original text is searchable
  ...
  is therefore `simple` with an exact-substring confirmation. Tests:
...
- `QC-AC-002` — spec `20:182`: "Save acknowledgment does not wait for
  ...
  cannot discard. What is measured instead is that `P-16`'s confirmation is
  recorded **before** `P-15`. Tests: `tests/pipeline/test_save_does_not_wait.py`.
...
| D-90 | Exact capture search is a **second** FTS plane over `capture_versions.content` using `simple`, not `english`, with exact-substring confirmation | One `capture_text_in_scope()` predicate **and** one statement builder parameterised over a `SearchPlane` — the shape `D-76`/`D-77` established for `JobPlane`, so the page and the totals agree **by construction** rather than by comparison. That control is the point: `coverage_for` and `match_statement` on the extraction plane were *asserted equal* for six review rounds and were false for two of six conditions. **Candidate (a) is refused** — routing captures through `knowledge.extractions` needs an `enrollment_id`, which is `NOT NULL` with a foreign key on a table merged revision `8b3f5c17d904` creates, and that is `D-76` on the identical column shape. **Why `simple`**: measured, it keeps stop words, does not stem, and matches `RFI-0421` and `$12,500.00` as adjacent lexemes; `english` fails `QC-AC-050` in two measured ways (`D-89`). **The cost is disclosed, not smoothed over**: no stemming, so a search for `meetings` will not find `meeting`. That is published as a `Limitation` through the disclosure envelope — `capture_search_matches_words_as_written` — rather than left for a caller to discover. Plus a larger index and a second predicate. **One defect this decision's own implementation carried and WP-7's acceptance tests caught**: the exact-substring confirmation compared bytes while the predicate it confirms compares lowercased lexemes, so a single-term query for `buyout` silently removed a capture whose text says `Buyout` — the criterion false for every capitalised word, with no exception anywhere. Both sides are now case-folded | Adopted and implemented in WP-7. Invalidated by a measured need for stemming on capture text, which would need its own plane and its own disclosure rather than a configuration change to this one |
