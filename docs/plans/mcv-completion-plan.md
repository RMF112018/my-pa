...
sections *Definitions*, *Supporting records*, and *State patterns*, with one
exception stated where it occurs: `09` lists `SourceSpan` and `SourceRegion` as
...
sections *Text spans* and *Page regions*.
...
alone. It was removed. Where a target below rests on a definition, the note says
...
domain; PRIE historical" — as `Canonical`, and the ratified information
executive description makes Relationship Intelligence "the people-centered
...
contract input. See `D-20`, which also records that the ratified architecture
