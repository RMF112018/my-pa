...
| Alembic head | `d8f3a1c6e942` in the repository, seventy-seven revisions; the chain admits GSQS start/status at `c4b0a1d9e827` immediately before Phase B continues at `c7a1f04b9e63`, then `gsqs.step` after `3d07af4dc513`; local validation targets disposable databases only | `migrations/versions/*.py`, `alembic heads` |
...
Two hundred and ninety-eight Python modules under `src/my_pa` and three hundred and ninety-two test modules —
