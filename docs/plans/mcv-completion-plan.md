...
Three hundred and fifty-two Python modules under `src/my_pa` and five hundred and eighty-one test modules —

**Current-tree remeasurement (CCA-005 / WP-TUX-01, rebased onto `origin/main` `57f3f92d`):** the two commands below count 352 source modules and 581 test modules on this tree. This change adds `tests/schema/test_legacy_direct_principal_task_origin_migration.py` and no source module under `src/my_pa`. The prior 579-module measurement remains below as historical evidence for the PR #286 tree.
...
**Historical merged-tree remeasurement (2026-09-23):** the two stated commands derive 352 source modules and 578 test modules against the tree produced by merging `origin/main` `438344b64380cd80cc82e29756103046289a5c31` (PR #280) into R01-WP08 `c4a7f39b08b00dd062e3ced051a6d4154be0e78f`. Neither side's pair is true of the merge: `main` recorded 577 and WP08 recorded 575, each true of its own tree, and the merged figure is neither, nor the sum of their deltas (RULING-M2). WP08 adds one module, `tests/capture/test_conversation_project_lineage.py`, the Conversation Project lineage proof that executes the real capture pipeline worker and the real review promotion; source modules are unmoved because the package adds none under `src/my_pa`. Counted against the merged tree with every conflict marker gone. The 2026-09-22 measurements at prior heads `55e26b792d421270c5446a6a776da3787fd2ab5d` and `70e300f3a97ca4f6981832c0f0ef21dcdfc0a78e`, and the dated provenance that follows, are historical evidence for their respective trees, not claims about this current tree.
`find src/my_pa -name "*.py"` and `find tests -name "test_*.py"`. Re-measured on 2026-09-19 by merging `origin/main` `8585a98e` (PR #264, R01-WP06 Portfolio Constraint read capabilities) into WP-POSTUX-06 (`bf/postux-wp06-today-derivation-20260917`, PR #266): neither side's pair is true of the merge — each recorded 572 test modules, true of *its own* tree, and each reached 572 from 570 by adding two different modules. Source modules 352 and test modules **574**, counted with the two commands against this merged tree with every conflict marker gone, never by adding the two deltas together (RULING-M2). Source modules are unmoved because neither package adds one. Re-measured on 2026-09-19 by WP-POSTUX-06 (PR #266, canonical Today Task membership contract, merged onto `origin/main` `ce63a83b`): source modules unmoved at 352 — the work package adds no module under `src/my_pa`, only a four-line docstring on `ApplicationService`; test modules 572 (was 570) after adding `tests/database/test_home_work_today_parity.py`, which proves Home's and Work's parameterizations of `tasks.list` answer one identical Today set, and `tests/database/test_today_predicate_fake_agreement.py`, which holds the fake task repository in `tests/conftest.py` to the SQL predicate it had been silently duplicating. Counted with the two commands against this merged tree (RULING-M2). Re-measured on 2026-09-15 by R01-WP06: source modules unmoved at 352; test modules 572 (was 570) after the package's two Constraint portfolio test modules (`tests/unit/test_constraint_portfolio_reads.py`, `tests/database/test_constraint_portfolio_reads.py`). R01-WP06 added no source module and no test module of its own — it extended existing modules — so the movement here is the two its read-plane half added and did not count. Counted with the two commands against this tree, run twice with `PYTHONDONTWRITEBYTECODE=1` and both passes agreeing, never inferred from the modules the package added (RULING-M2). Re-measured on 2026-09-15 by R01-WP05: source modules 352 (was 351) after adding `application/constraint_settings.py`; test modules 570 (was 564) after adding the four Project Controls settings test modules and picking up the two `b43e9b0a` left uncounted (`tests/database/test_project_controls_settings_persistence.py`, `tests/unit/test_project_controls_settings_history.py`). Counted with the two commands against this tree (RULING-M2). Re-measured on 2026-09-15 by merging `origin/main` `b783f9b1` (PR #257 ChatLLM full-data profile, PR #259 WP-POSTUX-03) into R01-WP04B (`bf/pc-cm-run01-wp04b-capture-project-20260914`, PR #258): neither side's pair is true of the merge — this branch recorded 349 source modules and 561 test modules, true of *its* tree; `origin/main` recorded 351 and 562, true of *its*. Source modules 351 and test modules 564, counted with the two commands above against this merged tree with every conflict marker gone, never inferred from the modules either side added (RULING-M2). Re-measured on 2026-09-14 by R01-WP04B: source modules unmoved at 349; test modules 561 (was 559) after adding the two WP04B test modules. Counted with the two commands against this tree (RULING-M2). Re-measured on 2026-09-14 by ChatLLM full-data profile: source modules 351 (was 349) after adding `domain/identity/chatllm_capability_policy.py` and `application/chatllm_data_profile.py`; test modules 562 (was 559) after adding three FAST contract/unit modules. Counted with the two commands against this tree (RULING-M2). Re-measured on 2026-09-14 by R01-WP03: source modules unmoved at 349; test modules 559 (was 558). Counted with the two commands against this tree (RULING-M2). Re-measured on 2026-09-13 by WP-MCP-PROJ-07 (Project MCP protocol harness plus WP-MCP-PROJ-06 admission contract): source modules unmoved at 349; test modules 558 (was 556). Counted with the two commands against this tree (RULING-M2). Re-measured on 2026-09-13 by WP-MCP-PROJ-05 (task Project reassignment; includes WP-MCP-PROJ-04's two uncounted modules): source modules unmoved at 349; test modules 556 (was 552). Counted with the two commands against this tree (RULING-M2). Re-measured on 2026-09-08 by merging `origin/main` `a541bebd` (PR #223) into PR #210 (`bf/r11-preserved-backup-current-firewall`): source modules unmoved at 345; test modules 539 (was 538) after adding `tests/architecture/test_nas_preserved_backup.py`. Counted with the two commands against this tree (RULING-M2). Re-measured on 2026-09-07 by merging `origin/main` `9bdf04e6` (PR #222) into AUTH-IMP: neither side's pair is true of the merge; counted with the two commands against this tree (RULING-M2). Re-measured on 2026-09-07 at UI-IMP-WP29, which adds two architecture modules (`tests/architecture/test_production_delivery_config.py`, `tests/architecture/test_frontend_public_browser_ingress.py`) and no source module, by running the two commands above against this tree — never by adding the deltas to the figures below (RULING-M2, FINDING-M1). Re-measured on 2026-09-06 at UI-IMP-WP28 (`bf/ui-imp-wp28-release-gates`), which adds one FAST unit module (`tests/unit/test_frontend_release_gates.py`) and no source module, by running the two commands above against this tree — never by adding the deltas to the figures below (RULING-M2, FINDING-M1). Re-measured on 2026-09-06 at UI-IMP-WP27 (`bf/ui-imp-wp27-test-protection-hardening`), which adds three FAST unit modules (`tests/unit/test_frontend_protection_ledger.py`, `tests/unit/test_frontend_ci_classify.py`, `tests/unit/test_frontend_synthetic_fixture_hygiene.py`) and no source module, by running the two commands above against this tree — never by adding the deltas to the figures below (RULING-M2, FINDING-M1). Re-measured on 2026-09-06 at PC-CM-IMP-WP03 (rebased onto `origin/main` `3f5c80f9`), which adds two source modules (`domain/project_controls/read_models.py` and `application/constraints.py`) and fourteen test modules (one unit, eleven database, two architecture), by running the two commands above against the rebased tree — never by adding the deltas to the figures below (RULING-M2, FINDING-M1). Corrected on 2026-09-04 to **three hundred and eighteen and four hundred and fifty-nine** by merging `origin/main` `455a3671` (PR #181, RI-ENT-WP-13, whose merge already carried PR #186) into `ri-ent/wp12-backfill`: `origin/main` recorded three hundred and eighteen and four hundred and fifty-seven, true of *its* tree at `12d6c698`; this branch recorded three hundred and eighteen and four hundred and fifty-two at `92bb5e56`, true of *its*; neither is true of the merge. Re-measured by running the two commands above against the merged tree with every conflict marker gone, never by summing the two deltas (RULING-M2, FINDING-M1). The WP-13 closeout that produced four hundred and fifty-seven itself recorded: Corrected on 2026-09-04 to **three hundred and eighteen and four hundred and fifty-seven** by merging `origin/main` `37f767b8` (PR #186) into `ri-ent/wp13-fixture`: `origin/main` recorded three hundred and eighteen and four hundred and fifty-four, true of *its* tree; that branch recorded three hundred and eighteen and four hundred and fifty-three at `62ff47f5`, true of *its*; neither was true of that merge. PR #186 adds four test modules under `tests/db/` — `test_provisioning.py`, `test_provisioning_guards.py`, `test_provisioning_unit.py`, `test_transactional.py` — and no source module. The figures
...
`ri-ent/wp09-resolution`/`origin/main` merge, and it was right for that tree. **Moved on 2026-09-06 by PC-CM-IMP-WP06 (`pc-cm-imp-wp06-mutation-plane`, rebased onto `origin/main` `5ae27186`):** three hundred and thirty-nine (was three hundred and thirty-seven) and five hundred and eleven (was five hundred and eight), for `application/constraint_management.py`, `domain/project_controls/relationship.py` and the three WP06 test modules. Re-measured with the same two `find` commands on the rebased tree, never a delta (RULING-M2). **Moved on 2026-09-06 by PC-CM-IMP-WP04 (`pc-cm-imp-wp04-read-capabilities`, rebased onto `origin/main` `86e703b7`):** five hundred and thirteen test modules (was five hundred and eleven), for `tests/schema/test_constraint_read_capability_migration.py` and `tests/unit/test_constraint_read_commands.py`; the `src/my_pa` figure is unmoved because this work package adds no module there. Re-measured with the same two `find` commands on the rebased tree, never a delta (RULING-M2).
