...
Three hundred and thirty-seven Python modules under `src/my_pa` and five hundred and ten test modules —
...
`ri-ent/wp09-resolution`/`origin/main` merge, and it was right for that tree. **Moved on 2026-09-06 by PC-CM-IMP-WP04 (`pc-cm-imp-wp04-read-capabilities`, rebased onto `origin/main` `5ae27186`):** five hundred and ten test modules (was five hundred and eight), for `tests/schema/test_constraint_read_capability_migration.py` and `tests/unit/test_constraint_read_commands.py`; the `src/my_pa` figure is unmoved because this work package adds no module there. Re-measured with the same two `find` commands on the rebased tree, never a delta (RULING-M2).
