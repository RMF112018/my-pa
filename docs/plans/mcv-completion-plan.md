...
| Alembic head | `9b2d5f8c3e01` in the repository, forty-one revisions; local validation targets disposable databases only | `migrations/versions/*.py`, `alembic heads` |
...
One hundred and ninety Python modules under `src/my_pa` and two hundred and twenty-three test modules —
...
| Alembic revisions — schemas and extensions, target tables, control plane, indexes, foreign keys, views, `knowledge` schema, extraction tables, audit events, enrolled objects, continuity, native sources, managed documents, GoodNotes and operations | Implemented, forty-one revisions, head `9b2d5f8c3e01` |
