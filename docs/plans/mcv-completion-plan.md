...
One hundred and nineteen Python modules under `src/my_pa` and one hundred and nine test modules —
...
- Smaller, and named so they are not rediscovered: the `extractions` check
  ...
  *not* a row here" 57 lines above a constraint that admitted exactly such a row
  ...
  emitted constraint text with the live declaration. **The "FAST-green and
  unvalidated where it matters" sentence that stood here is superseded by
  `D-109`**: the database tier ran on 2026-08-09 against a disposable PostgreSQL
  17.10 instance at the head `D-109` records, which is also where its
  selection and counts are kept rather than restated here, so the refusal in
  `test_the_schema_refuses_a_quarantined_outcome_filed_as_an_extraction` is now a
  measurement. Running it is also what showed the `downgrade` described above as
  "a frozen literal each way" to be restoring a vocabulary no database at the
  revision below it holds — the defect `D-109` records. **Four of
...
| D-108 | **`extractions.status` is narrowed to the vocabulary a writer can file, and one of `8b3f5c17d904`'s five allowlisted derivations is closed while the other four remain `D-81`'s reserved package** | The declaration's own comment said "a quarantined object is *not* a row here" **57 lines above** a constraint that admitted exactly such a row: `extraction_status_is_known` was `_one_of("status", ExtractionStatus, ...)`, derived from the three-member outcome enum. That is a rule stated in prose and enforced by nothing, inside one declaration — the `D-69`/`D-81` shape. The **outcome** vocabulary is legitimately three and stays three: `record_outcome` dispatches on all three and `outcome_for_object` reports a quarantine from `quarantine_records`, so the handed-down framing that the status was "uncounted" was **measured false**. The **storage** vocabulary is two, because `record_outcome`'s quarantined branch returns a `quarantine_records` row before the single `pg_insert(extractions)` in `src/`. Narrowing replaces a *demonstration* that a mis-filed row is not counted with an *impossibility*. **Written as an inline literal at the call site, deliberately not as a module-level named `frozenset`** — measured, not reasoned: a module-level `frozenset({"extracted", "unsupported"})` is discovered by the `D-81` guard's `_live_closed_sets` walk and puts the site straight back into `_derived_sites()`, leaving the merged revision tracking a renamed constant instead of an enum. As an inline literal the emitted set equals no live closed set (89 live closed sets in `my_pa` at this head; `{extracted, unsupported}` is not among them), so the site drops out by measurement and `ALLOWED` shrinks **15 to 14** — the direction the guard requires. **The disclosure, which must not be read as the reserved package being done.** This freezes **exactly one of `8b3f5c17d904`'s five** allowlisted sites, and it does so through the *declaration* rather than through `_FROZEN`. The other four — `only_a_supported_media_type_is_extracted`, `quarantine_reason_is_known`, `quarantine_review_state_is_known`, `limitation_reason_is_known` — are untouched, still derived, and still belong to the package `D-81` reserves for freezing `7e5a1fb93d62` and `8b3f5c17d904`. That package is neither done nor made unnecessary by this row; `assert not {revision for revision, *_ in ALLOWED} & set(FROZEN)` still makes `8b3f5c17d904` all-or-nothing for the `_FROZEN` technique, which is why this route was taken instead. **`9d4e7a3b1c62` is what keeps the two ways to head honest**, since `8b3f5c17d904` creates `extractions` from the live declaration and a fresh database therefore gets the narrow constraint at that revision while a migrated one keeps the wide one. It restates the constraint forward on `2b7e9f4c1a83`'s `_restate` pattern, unconditionally, with a frozen literal each way and a real `downgrade`; a pre-existing `quarantined` row makes `ADD CONSTRAINT` fail loudly rather than being deleted to let the migration through. **What replaced the lost derivation**, because cutting it removed the only thing that noticed a fourth outcome member: `test_the_stored_status_vocabulary_is_the_outcome_vocabulary_less_quarantined` asserts the storage vocabulary equals `{m.value for m in ExtractionStatus} - {QUARANTINED}` against a derived right-hand side, and `test_a_migrated_database_converges_with_a_freshly_built_one` compares the last emitted constraint text against the live declaration. Both were proven to fire with the defect reinstated. The read-side `status == 'extracted'` filter in `extracted_text_in_scope` becomes **redundant-but-correct and is retained, not deleted**: every row it excludes is `unsupported` under the same enrollment and so already excluded by the sibling `not_in`, whose columns are both `NOT NULL` and cannot go three-valued — but the redundancy rests on a `CHECK` rather than on anything the rows express, so the condition would resume deciding the moment that `CHECK` were widened | Implemented on `bf/extractions-quarantined-debt`. **The `database` tier has now run and this row's earlier "unvalidated where it matters" no longer holds** — see `D-109`, which also records that the `downgrade` this row describes as "a frozen literal each way" was measured wrong. Run 2026-08-09 against a disposable PostgreSQL 17.10 instance, at the head recorded in `D-109`: **453 selected, 453 passed**, so the refusal in `test_the_schema_refuses_a_quarantined_outcome_filed_as_an_extraction` is a measurement rather than an inference from the constraint text. Invalidated by a fourth `ExtractionStatus` member, which must state which vocabulary it joins, or by the reserved freeze package, which must then account for the four rows this change left |

| D-109 | **A `downgrade` must restore what the revision below it *denotes*, which is not the same as what it denoted; a precedent is a technique plus its precondition, and copying only the first is what produced the defect** | `9d4e7a3b1c62`'s `downgrade` restored `status IN ('extracted', 'quarantined', 'unsupported')` on `2b7e9f4c1a83`'s `_restate` precedent. **Measured, not argued:** two disposable databases built against PostgreSQL 17.10, one `upgrade 7f2a9d6c4e18` from empty and one `upgrade head` then `downgrade 7f2a9d6c4e18`, ended at the same revision holding `CHECK ((status = ANY (ARRAY['extracted'::text, 'unsupported'::text])))` and `CHECK ((status = ANY (ARRAY['extracted'::text, 'quarantined'::text, 'unsupported'::text])))` respectively — the exact divergence `D-108` says `9d4e7a3b1c62` exists to remove, reintroduced by its own reverse. **The precondition, which is the general lesson and not a fact about this revision.** `_CAPABILITIES_BEFORE_THIS_REVISION` is sound in `2b7e9f4c1a83` because the revision below *it* carries a frozen literal, so its denotation cannot move. Here the revision below is `8b3f5c17d904`, which builds `extractions` from the live declaration and had **already moved** when the declaration was narrowed. The technique was copied and the condition under which it is sound was not checked. `1a4c9e77b2d5` also carries a companion the copy left behind — a database-tier test pinning what its own downgrade restores — and that absence is why nothing caught it. **The fix restores the narrow set rather than freezing `8b3f5c17d904`**: freezing is `D-81`'s reserved package and `assert not {revision for revision, *_ in ALLOWED} & set(FROZEN)` makes that revision all-or-nothing across the `ALLOWED` rows it still carries, beside the `assert len(ALLOWED) == 14` that fails if that moves; `D-108`'s "five" counts them before this package narrowed one, and is a past figure rather than a current one. One constant now serves both directions, and an empty `downgrade` was rejected even though it reaches the same database, because emitting no DDL leaves the guard below nothing to read. **Corrected as a class, with the universe named.** Every `CHECK` any `downgrade` in the chain restates: `extraction_status_is_known` by `9d4e7a3b1c62`, `capability_is_known` by `3c8f1e2a5b74`, `2b7e9f4c1a83` and `1a4c9e77b2d5`, and `purpose_is_known` by `3c8f1e2a5b74` and `1a4c9e77b2d5`. Every one that is not `9d4e7a3b1c62`'s already emitted the correct text — **correct is not guarded**, and only `1a4c9e77b2d5`'s pair was guarded, by a database-tier test. `zz_reviewer_probe` planted in `2b7e9f4c1a83`'s and in `3c8f1e2a5b74`'s downgrade constants left the whole FAST tier at **2995 passed, green** with the new module deselected, and **1 failed, 2996 passed** with it selected. **The first version of that class guard was itself defective, and an independent review broke it rather than this worker finding it.** Its landing-text lookup was keyed on constraint *name alone*. Seven `CHECK`-carrying names in this chain sit on more than one table — nine relations do, counting two primary keys — `state_is_known` on `knowledge.jobs` (`failed, queued, running, succeeded`) and on `migration_control.table_progress` (`RUNNING, COMPLETED, FAILED`), `status_is_known` on two `migration_control` tables, and five `*_is_an_opaque_identifier` names across the capture and audit tables — so a `downgrade` restoring `table_progress.state_is_known` with `jobs`' vocabulary passed the guard **green at 2 passed** while two real databases at `7f2a9d6c4e18` diverged. That is the very class the module says it closes, exhibited with the module green: **a fix reproducing its own defect inside itself**, for the fifth time in this campaign. Both sides are now keyed on `(table, constraint)` and the reviewer's plant reddens, with the correct text at the same site as the control. Two further weaknesses the same review measured are closed with it: `KNOWN_RESTATED` was a **floor**, and deleting a row from it left FAST green — it is now an **equality**, so a restatement entering the chain or leaving this list both redden; and a `downgrade` landing on `base` compared against the head-most text rather than against nothing, which is now refused outright. **The README and this row both stated the property as whole-schema convergence and now state it at what is checked**: `CHECK` text on constraints a downgrade re-adds, keyed by table, with the module's limits section naming what it leaves out — `CHECK` constraints only, re-added constraints only, and nothing about column types, indexes or triggers. **A third review broke it again, and the third defect is the one that generalises.** The guard's reader is a regular expression, and it *skipped* what it could not match instead of failing on it. A wrong restoration written `CHECK(` without the space, or as an unnamed `ADD CHECK`, or in lower case, or with a newline before `CHECK`, produced no row, was compared against nothing, and left the module **green at 2 passed** — measured in all four shapes with a canonical-syntax control reddening at 2 failed beside them. A fifth shape came from the other end: `_BOUNDARY` matched revision identifiers as `[0-9a-f]*`, so a revision created with a non-hex id produced a boundary line the pattern missed, `re.split` did not split, and that revision's entire DDL went into the discarded preamble. **The lesson is not "widen the regex", because the next unanticipated shape is not in the list either.** A parse that skips is worse than a parse that is narrow: narrowness is visible in a limits section and skipping is visible nowhere. So the reader is now audited by something cruder than itself — `_ANY_CHECK` counts every `CHECK` in every downgrade block and the reader must have read all of them — and the block count is asserted against `migrations/versions/`. All six shapes redden; a probe restoring the *correct* vocabulary still passes the comparison and fails only the unlisted-restatement equality, which is the intended signal. Two prose defects the same review found are corrected with it: the module docstring still carried the superseded "floor" framing 108 lines above the constant that negates it, and the limits section named a symbol `_CONSTRAINT` that does not exist. **Three review rounds, three defects, all three in the guard rather than in the migration** — the migration's fix was right the first time and the thing built to prove it was wrong three times. **A fourth review broke it again, and that is what settled the design.** The auditor added in round three was `\bCHECK\s*\(` and the reader's own constraint alternative degenerated to the same sub-pattern, so the two could not disagree: a `CHECK` the reader could not see was a `CHECK` the auditor did not count, the assertion compared `0 == 0`, and the whole thing was a tautology. Measured: a wrong `purpose_is_known` restoration on `3c8f1e2a5b74` written `CHECK/*restore*/(` left the guard green, FAST green at 2997 and **the database tier green at 452**, while two real databases at `2b7e9f4c1a83` held different vocabularies. **Four rounds, four defects, every one of them a way the parse was incomplete and none of them in the migration** — whose fix has been verified correct by execution three times and faulted zero times. The class is "a text nobody anticipated", and no regular expression closes it. **So the parser was deleted rather than repaired for a fifth time.** The claim now runs the migrations and asks PostgreSQL what it holds: for every revision, the constraints a freshly built database has at the parent versus the constraints one has after going up and back down — every constraint in every schema, `pg_get_constraintdef`, nothing enumerated. It costs a few seconds in the database tier, it subsumes every plant from all four rounds including the two comment shapes that defeated the parser, and it needs no list, no vocabulary regex and no `KNOWN_RESTATED`. It also puts the claim where `README.md` already said migration behaviour is checked; siting it in FAST was the mistake underneath all four rounds. **And a fifth review found the first version of it reading `pg_constraint` alone**, which misses `NOT NULL` (`pg_attribute.attnotnull`) and `DEFAULT` (`pg_attrdef`) entirely: a `downgrade` that dropped a `NOT NULL` left two databases at one revision disagreeing about whether a column could be null — `D-109`'s own shape — with the whole tier green, and a stray index survived the same way. The snapshot now reads four kinds of fact: constraints, columns with type, nullability and default, indexes, and non-internal triggers. All three plants redden. **What it does not compare is stated at capability and no longer handed to a test that does not check it** — the earlier wording gave column types, indexes and triggers to `test_head_round_trip.py`, which reads `relkind IN ('r','p','v','m','S','f')` and so excludes indexes, and reads relation names rather than column types or triggers. Outside the four kinds, and outside every test: comments, privileges, ownership, collations, sequence parameters, non-trigger functions, row-level security policies, publications, and rows. **A sixth review found the four-kind snapshot still short by three**, and the shape is worth recording because it is the fifth-round defect with its sign reversed. Round five blocked because prose handed column types, indexes and triggers *to* `test_head_round_trip.py`, which does not read them; the correction then handed *that* test's own property — a stray schema, extension or relation left behind — to this module, which did not read those. Measured: a `downgrade` leaving a stray view, schema, sequence or extension behind passed green, and at an intermediate revision that residue was outside every test in the tree, since `test_head_round_trip.py` only compares `head` against `base`. The snapshot now reads seven kinds and all four plants redden. Two smaller corrections went with them: the `alembic_version` exclusion covered the table but not its index, which is a separate `pg_class` row and failed the first revision once relations were compared; and the note claiming the exclusion hides "one fact" was written for the constraints-only version — it hides five, measured. **The `D-81` guard's docstring claim that "a new derived constraint appears in any revision — red" was false for `op.execute`** and is corrected rather than qualified away: an `op.execute` joining `ExtractionStatus`'s members into a `CHECK` reached the rendered DDL verbatim and left `test_no_revision_derives_a_closed_set_from_an_enum.py` at **16 passed**, its unplanted count. The gap is occupied by a merged revision, not only by a plant: `7f2a9d6c4e18` builds all seventeen of its tables in raw SQL, imports only `alembic.op`, sits in neither `ALLOWED` nor `FROZEN`, and has `_emitted() is None`, so the readability test skips it; its DDL carries sixteen `_CLOSED_SET` matches, one naming columns rather than literals and discarded, leaving seven distinct literal vocabularies, **three of which equal a live closed set exactly** (`ResolutionAction`, `EvidenceAuthority`, `personal_fixture._ALLOWED_DOMAINS`). Read by hand it derives nothing, so it complies with `D-69`; what is missing is verification, not compliance | Implemented on `bf/extractions-quarantined-debt`. The `database` tier ran green at the head this row was written against — **453 selected, 453 passed, 289.33s**, the walk itself about seven seconds of that — on a disposable PostgreSQL 17.10 instance on loopback, created and removed for the run, never the canonical `my_pa`, which was left holding exactly `my_pa`, `postgres`, `template0` and `template1`. FAST there is **2995 passed, 453 deselected, 50.57s**, two fewer than before because the parsed guard was deleted and its claim moved to the tier; `ruff`, `ruff format --check` over 480 files and a bare `mypy` over 141 source files are clean. The figures are dated to that head rather than described as standing, because the tier had never run for this package at all and a coverage claim was what `D-108` had to correct. **Closing the raw-SQL blindness is explicitly not done here** — it needs a rule that reads each revision's emitted SQL text and attributes vocabularies to live closed sets, which is a different parse from the object-graph read and belongs to a package that owns that guard. Invalidated by `8b3f5c17d904` being frozen, which would make the revision below stop moving and change what this row's fix is for; or by a schema dimension moving outside the seven the walk compares, which the module's limits section lists |
...
**Recomputed 2026-08-09 for `D-109`, and the reservation still held.** The correction cycle on the extraction-constraint package needed a row for a decision, so the parse was run a third time rather than inherited: at `6667d70` this table held **106 rows, `D-1` through `D-108` with the deliberate `D-106`/`D-107` gap**, and the identical parse over `origin/recovery/pre-20260805-utc-rollback-c9fb513`, the `custody/wp12-local-main-88e8d81` tag and the `custody/pre-20260805-rollback-line` tag returned **107 rows through `D-107`** on all three, with `D-109` present on none of the four. So `D-109` was free and is minted; `D-106` and `D-107` remain spoken for and are still not readmitted.
