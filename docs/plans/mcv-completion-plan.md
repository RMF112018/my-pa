...
| Alembic head | `af3d35efb9c0` in the repository, ten revisions; the canonical database remains at `6c4d3ea82f10` because the four `knowledge` revisions run only against disposable databases | `migrations/versions/*.py`, `select * from alembic_version` |
...
| D-65 | The `internal_error`-on-schema-drift taxonomy fix is **out of** WP-5B's scope. Named, deferred, and not absorbed | WP-5B's probe *detects* the condition; correcting the application's error classification for it is a separate change against `../../src/my_pa/application/errors.py` and the three transports' negative-evidence matrices. The WP-5B design flagged it as tempting and refused to absorb it, which is the behaviour `D-57` asked its own design for. It is a real defect of the same class `unit_of_work.__enter__`'s docstring records WP-4B2a fixing for the *unreachable* case — "a caller told the product is broken when the truth is that PostgreSQL is not running, which are different things to do about it." The unreachable case was fixed; the schema-drift case was not. Absorbing it would widen WP-5B past its in-scope list and would touch `SPEC-AC-001` parity evidence across three transports. **Recorded so it cannot be rediscovered as a surprise**, exactly as `D-58` was recorded for WP-5B | Deferred and named. Invalidated by a package taking it, which needs its own decision row and acceptance criterion |
