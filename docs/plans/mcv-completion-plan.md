...
exist, and `:250` settles the precedence: "Items 4 through 13 restate section
12's sequence table, which is the authoritative one." The two remaining Remote
...
**Four of these were wrong or narrowed against the spec and are corrected in
...
- `QC-AC-010` immutability **and independent retrievability** — a domain test
  ...
  covered immutability only; spec `:189` also requires "and independently
...
- `QC-AC-012` distinct timestamps — **five**, not three: `client_created_at`
  (device), `server_received_at` (server), `occurred_at`, `recorded_at`
  (processed), and `accepted_at`, separately stored and never substituted for one
  another, with a genuinely absent client time stored as `NULL` rather than
  invented. *Corrected: this named three and silently dropped **occurred** and
  **accepted**. Spec `:191` requires all five to remain distinct, so the
  paraphrase narrowed the criterion by two columns.*
...
- `QC-AC-031`/`QC-AC-032` idempotency — replaying an identical request returns
  ...
  closed set of **eleven** public codes and that is not one of them; inventing a
  twelfth would be an unauthorised expansion of `v1`.*
- `QC-AC-034` — an induced audit or receipt failure fails the whole transaction
  ...
  *Unchanged, and deliberately so: this is right even though the mechanism
  ...
  would be asserting `D-34` is false.*
- `QC-AC-041` — a redaction test asserting no capture text reaches logs, audit
  rows, error payloads, **or URL parameters**, and that the text **is**
  retrievable through `capture.read`. *Corrected: this named three sinks. Spec
  `:214` also names telemetry, URL parameters, and lock-screen notifications, and
  URL parameters is live here — `capture.create` over HTTP must not accept text in
  a query string. The retrievability clause is what stops a build that stored
