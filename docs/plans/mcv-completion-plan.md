...
- Smaller, and named so they are not rediscovered: the `extractions` check
  ...
  **offline `--sql` review artifact alone**:
  ...
  library is available to this repository — it raises on any construct it does
  not understand rather than skipping it, which is the property that keeps a
  parse failure from reading as a pass. The superseded wording is kept and
