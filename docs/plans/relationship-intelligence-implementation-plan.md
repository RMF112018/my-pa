...
| Types | `mypy` (configured targets: `src`, `migrations`, `apps`, `ops`) — clean over 407 files. Re-derived on the Phase B plus GSQS merge candidate from the configured target tree. |
| Full database tier green | `pytest -m "database or recovery or e2e"` against a disposable PostgreSQL 17.10 — **1,513 passed**, as one complete selection on the Phase B plus GSQS merge candidate. The figure equals collection, so nothing is skipped. Earlier Phase B and GSQS database figures remain valid only for their separate trees and are superseded here. |
