...
| Claim | Evidence |
|---|---|
| FAST tier collection and prior execution | **Collection only; no current runtime pass is claimed.** The repository's combined-tree collection census for `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` accounts for **9,295 passed-or-not-currently-executed nodes**. Architecture tier **4,014 passed-or-not-currently-executed nodes** plus the non-architecture remainder (5,281) partition that census exactly. Before this merge, the GoodNotes image-content exact head executed 9,080 nodes successfully (3,976 architecture plus 5,104 remainder), the RouteLLM exact head executed 9,071 nodes successfully (3,976 architecture plus 5,095 remainder), and the independently reviewed WP-FE-03 tree executed 9,113 of its 9,115 nodes successfully (3,926 architecture plus 5,187 remainder); its other two Trio-dependent nodes were separately unavailable because the optional `trio` dependency was not installed. Those executions are historical evidence for their respective exact trees and are not asserted for this combined tree. The collection equality remains enforced, while execution and environment availability are stated separately. |
| Lint and format | `ruff check .` and `ruff format --check .` are clean on this combined tree. **No file count is stated**, because `ruff` walks the working directory and its corpus size differs between a developer checkout and a runner; `mypy`'s count below stays bound because it is derived from configured targets. |
| Types | `mypy` (configured targets: `src`, `migrations`, `apps`, `ops`) — clean over 368 files on this combined tree. |
| Full database tier | `pytest --collect-only -m "database or recovery or e2e"` selects **991 tests** at this combined tree. Runtime execution was not repeated for this merge-conflict resolution. No shared or production database was used. |
