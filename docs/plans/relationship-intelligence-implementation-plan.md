...
| MCP surface | `tests/contract/test_mcp_transport.py` — a real child process composed for the plane and its write half publishes all 95 tools, and an unconfigured one publishes 53, withholding the thirty-one `entities.` names along with the six `documents.` names and the nine `relationship_memory.` names. Corrected 2026-08-23: this cell said 54, which was the whole capability set before the Intelligence Artifact and Relationship Memory planes and is now neither of the two figures this test asserts. Neither number is written into the test — both are derived from `Capability` there, which is why the test stayed green while the cell describing it went stale |
