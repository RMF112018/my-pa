...
| **WP-RI-06** — observation, proposal, review, and merge workflows; review integrated into the *existing* review plane; merge preview; transactional redirect; audit and receipt; replay protection; expected-version checks; scheduled agents excluded from merge apply | A governance service reachable by some caller, whose review path is the repository's one review mechanism | **Completed by Phase B and final completion.** Entity and Relationship Memory candidates appear on canonical `review.list` and all governed dispositions run through `review.decide`; accepting an identity-correction proposal does not mutate identity. Operator-only merge and split each require a persisted exact-version-bound preview and append immutable operation/effect history. Local and remote entry points are composed; remote correction remains off by default and requires global remote writes plus an exact server-resolved `remote.operator` durable capability set and all ordinary gates | Repository split execution and all nine v0.2 re-enrichment trigger registrations are now implemented. Frontend, live grants/flags, production/runtime commissioning, WP-08/WP-09 execution, deployment, and live data remain outside final completion |
...
final-completion campaign registers all nine from closed, existing authorized
mutation paths into Principal-scoped durable work in the same transaction. It
...
| Full database tier disposition | **FULL_DB_TIER_NOT_RUN_TARGETED_EVIDENCE_SUFFICIENT.** The exception criteria were not met: the campaign adds bounded RI recovery/ledger tables and focused isolated-PostgreSQL tests, while the execution environment has no `MY_PA_DATABASE_URL`. The `database or recovery or e2e` selection collects 1,532 tests on this tree; collection is not execution evidence. Targeted schema, migration, SQL-structure, concurrency, and database-test collection evidence is recorded in the campaign completion report. |
