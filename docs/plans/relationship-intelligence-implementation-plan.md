...
| Claim | Evidence |
|---|---|
| FAST tier green | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **7924 passed, 0 failed, 952 deselected** |
...
| Full database tier green | `pytest -m "database or recovery or e2e"` against a live PostgreSQL 17 — **942 passed, 0 failed**. Measured in three disjoint chunks — `tests/database` (204), `tests/schema` (338), and the remainder (400) — because the single-command run was killed twice mid-flight at 84% and 53%, and a partial run is not evidence. The chunks partition the same selection and sum to the 942 the last completed single run reported. Stated this way rather than as one number, because it was not one command. The four schema-suite failures adversarial review surfaced (accumulation sets in the audit, enrollment, capture and entity migration tests, outgrown by this plane's eight tables and five capabilities) are fixed, not deselected |
...
| 3 | `04a48ae` | see below | A sentinel window that was not before any moment, so a *cancelled* assignment corroborated at `as_of=datetime.min`; the currency clock unpinned at the capability; the evaluation corpus blind to the axis it was cited for; a concurrency race in `redirect_entity`; two false citations naming test files that have never existed |

**Twenty guards across three rounds were correct and untested**, not nine across
...
untested.** Twenty separate mutations survived the full suite — round 1 found
...
row is an answer, and `redirect_entity`'s guard was verified over all 252
three-step arrangements rather than the two that were reported.
