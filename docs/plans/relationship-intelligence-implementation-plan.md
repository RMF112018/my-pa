...
| Full database tier disposition | **FULL_DB_TIER_NOT_RUN_TARGETED_EVIDENCE_SUFFICIENT.** The immutable-origin correction used a separately verified, localhost-only, auto-removed PostgreSQL 17.10 container for 82 focused tests; the proposal-token correction recreated the same bounded target pattern for five unequal-version cases. Neither authorized or ran the full database tier. The `database or recovery or e2e` selection collects 2,073 tests on this tree, counted by `--collect-only` on the merged tree rather than taken from either branch: RI-ENT-WP-10/11 stated 1,551 and UI-IMP-WP02 stated 1,564, neither is true of a tree carrying both, and neither was true of its own tree either -- this figure is bound by no guard (the tier rule reads a `pytest -m "..."` command with a bolded pass count beside it, which this cell does not write) and had drifted accordingly. The 2,073 is 2,070 `database`-marked, 17 `recovery`, 16 `e2e`, overlapping; collection is not execution evidence for the unselected tests. Targeted schema, migration, SQL-structure, concurrency, and database execution evidence is recorded in the campaign completion report. |
