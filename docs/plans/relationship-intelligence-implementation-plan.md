...
and nine rounds have now run — dispatched to reviewers holding no part of this
...
**Nine rounds so far, not one.** Each is four reviewers, one per lens, run by
...
* **Carried, from the ninth review — three fidelity gaps in the in-memory
  double.** None is reachable through a transport, and each is the kind of
  divergence that lets a unit test prove something the server does not do.
