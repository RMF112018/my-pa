...
| Claim | Evidence |
|---|---|
| FAST tier validation | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — this selection collects 16,425 on this tree, so a green run is **16,425 passed**, 0 skipped, 2,106 deselected (18,531 total collected+deselected). **Moved on 2026-09-03 by the base integration of `origin/main` into `ri-ent/wp13-fixture`, in two steps: `8f0e4779` (PR #177, RI-ENT-WP-10/11, at `e004942b`, and PR #182, UI-IMP-WP07 through WP10, on top of it) and then `1bb7c3cf` (PR #183, UI-IMP-WP11)**: `origin/main` recorded 16,380 collected and 2,092 deselected of 18,472 at `8f0e4779` and 16,382 / 2,092 of 18,474 at `1bb7c3cf`, each true of *its* tree; this branch recorded 15,360 / 2,039 of 17,399, true of *its* — none is true of the merge, which carries RI-ENT-WP-10/11's modules and RI-ENT-WP-13's together. The first step measured 16,423 / 2,106 of 18,529 on the `8f0e4779` merge (head `903b8b15`); the second step moved the collected and total figures by exactly the two FAST-eligible tests UI-IMP-WP11 added to `tests/unit/test_intelligence_artifact_pipeline.py`, and the deselected figure not at all — measured, not inferred. Re-measured by `--collect-only` on the merged tree with every conflict marker gone, and again after this row had itself been rewritten, because the row is inside the tier it counts; never by summing the two branches' deltas (RULING-M2). Executed in full at this head, tree-wide by marker rather than by a directory list. Both branches' histories of this row are preserved below, this branch's first and then `origin/main`'s, each true of the tree that wrote it. **This branch's (RI-ENT-WP-13) history of this row, as it stood at `e94dd4b3`, preserved:** Corrected 2026-09-02 a third time, at RI-ENT-WP-13 closeout, from 15,215 to 15,240 with the deselected figure unmoved at 2,033: the closeout adds one architecture guard, `tests/architecture/test_a_project_name_is_never_a_global_identity.py`, which holds 24 FAST-eligible tests, none of them `database`-marked, and this table now states one new per-file `` `tests/….py` — N tests `` claim, which `tests/architecture/test_claimed_test_counts_match_collection.py` dynamically parametrizes into one further FAST-eligible verification test, so 24 + 1 = 25, which is the whole of this figure's movement and the whole of the total's movement from 17,248 to 17,273, with no residual. The deselected figure does not move because the closeout adds nothing under a deselected marker. Corrected 2026-09-02 before that, on merging `origin/main` into `ri-ent/wp13-fixture`: RI-ENT-WP-13 adds two test modules on top of everything below, and the figure is re-measured on that merged tree rather than added to. RI-ENT-WP-13's own attribution: passed from 14,984 to 15,002, and deselected from 1,974 to 1,988: RI-ENT-WP-13 added two test modules. **The four words "from 14,984 to 15,002" were restored at closeout** (`FINDING-M3`): this sentence read `attribution: , and deselected` until now. **The merge did not do this.** The controlling context performed the base merge that produced this head and resolved its conflicts programmatically, as a Python string operation on the file: `ours.split("Corrected 2026-09-02 from 14,984 to 15,002", 1)[1]` discarded the very phrase it split on. The loss was invisible to `ruff`, `mypy` and every test, because nothing here reads a sentence — the count guards compare figures to the tree, and the figures were all correct. **Never resolve a text conflict by string concatenation, and read the affected paragraph after any programmatic edit to a document.** Unlike the sibling instance in `mcv-completion-plan.md`, this one never reached `main`: it was created in this branch's own base merge and is repaired here before the branch leaves. They are `d09c6474`'s own words and they are also forced arithmetically by the two figures that survived — 16,958 − 1,974 = 14,984 and 16,990 − 1,988 = 15,002 — so the restoration is derived, not reconstructed from memory. `tests/unit/test_entity_record_family_security_matrix.py` holds 16 FAST-eligible tests, and this table now states two new per-file `` `tests/....py` — N tests `` claims, each of which `tests/architecture/test_claimed_test_counts_match_collection.py` dynamically parametrizes into one verification test at collection time — itself FAST-eligible — so 16 + 2 = 18, which is the whole of this figure's movement, with no residual. The deselected figure moves by exactly the 14 `database`-marked tests in `tests/database/test_tbr_completeness_fixture.py`, and 18 + 14 = 32 is the whole of the total's movement from 16,958 to 16,990. **What was measured, precisely:** both figures come from `--collect-only` against this exact tree *after* the edits that state the two new claims, stable across two consecutive collections. The **passed** figure is a collection-derived claim, not a whole-selection execution record: this head was produced while three sibling work packages held the campaign's machine-wide database-tier serialization gate, so the FAST selection was not executed end to end here. What was executed green at this head is the architecture tier in full and `tests/unit/test_entity_record_family_security_matrix.py` (16 passed). Prior correction 2026-09-02 from the two conflicting figures the RI-ENT-WP-09 merge brought together, to 15,197 collected and 2,019 deselected: `ri-ent/wp09-resolution` had corrected the row from 14,984/1,974 to 15,134/2,006 for RI-ENT-WP-09, and `origin/main` had corrected the same row from the same baseline to 15,047/1,987 for UI-IMP-WP02. Both corrections were right about their own branch and **neither is true of the merged tree**, which carries both sets of tests. **Re-measured by `--collect-only` on the merged tree — never by summing the two deltas**, which is the trap an independent reviewer named when it found this conflict by read-only `merge-tree` before the merge was attempted, and which is what RULING-M2 requires. Measured: 15,197/17,216 collected, 2,019 deselected -- and re-measured a second time after this row itself was rewritten, because the first measurement was taken while the merge's conflict markers were still in the file, which left BOTH branches' claims present and parametrized one verification test per side. That first reading of 15,206/17,225 was an artefact of the unresolved conflict, not of the tree. The two prior corrections are preserved below in date order, each true of the branch that wrote it. Prior correction 2026-09-02 (UI-IMP-WP02, `origin/main`) from 14,984 to 15,047, and deselected from 1,974 to 1,987: reconciling UI-IMP-WP02 onto current main added ten FAST-eligible tests (seven unit digest/session-authority cases and three schema declaration/chain checks) plus fifty-three architecture parametrizations over the five new identity-plane modules, which this selection also admits, and thirteen `database`-marked tests this selection deselects. 10 + 53 = 63, which is the whole of the collected movement; 13 is the whole of the deselected movement. This correction states no new per-file test-count claim in the bound form, so it adds no self-referential verification test. Measured directly by `--collect-only` against this exact reconciled tree (15,047/17,034 tests collected, 1,987 deselected). Prior correction 2026-09-01 (RI-ENT-WP-09, this branch) from 14,984 to 15,134, and deselected from 1,974 to 2,006: this tree now also carries RI-ENT-WP-09 (entity resolution/search vNext). The movement in both figures is attributed in full, by name, with no residual. Of the collected figure's +150: `bb3bf22` added `tests/unit/test_entity_resolution_vocabulary.py` (+75), `2efd2fc` added thirty-seven tests to the existing `tests/unit/test_entity_resolution.py` (+37), `1062251` added `tests/unit/test_entity_search_reaches_context.py` (+23), and `5700c37` added `tests/contract/test_entity_search_disambiguators.py` and `tests/unit/test_entity_search_disambiguators.py` (+15 between them). 75 + 37 + 23 + 15 = 150. Of the deselected figure's +32: `8f6c576` added `tests/database/test_entity_resolution_value_reads.py` and `1062251`/`5700c37` added and extended `tests/database/test_entity_search_reaches_context.py`, all `database`-marked and so excluded here. **Both were unexecuted when RI-ENT-WP-09's workers committed them, and both have since been executed by the controlling context.** Executing them found two defects static verification had not: `NameTypeCode.TRADING` never existed, so all eleven tests of `test_entity_resolution_value_reads.py` errored at setup and none had ever run (`f031c40`); and the widened search's active-state filter was held by nothing, proved by mutation and closed by two new tests (`b29340b`). Those two added tests are the +2 that moved this figure from the 2,004 first written here to the 2,006 measured now, and are why that first figure had a residual of 2 -- corrected by re-measurement, never by adding a delta. `0571b3e`, `dcae6bd`, `c0e36d7` and `2534a22` changed ports, doubles and evaluation fixtures without adding or removing a collected test, so none of the four moves either figure. **This correction states no new per-file `` `tests/….py` — N tests `` claim**, so it adds no self-referentially parametrized verification test to this selection or to the architecture tier, and the architecture figure below moved to 4,796 — not from RI-ENT-WP-09, which adds no architecture test, but from UI-IMP-WP02, which this merge brings in and which adds entries to two `tests/architecture/` guard registries. Measured, not assumed. Measured directly by `--collect-only` against this exact tree (15,134/17,140 tests collected, 2,006 deselected). **`origin/main`'s (RI-ENT-WP-10/11) history of this same row, as it stood at `e004942b`, preserved -- its lead figure is restated in prose rather than in the bound form, so that the tier rule binds only the figure at the head of this cell:** that row's selection collected 16,382 on `origin/main`'s tree at `1bb7c3cf` (16,380 at `e004942b` and `8f0e4779`), so a green run there was 16,382 passed, 0 skipped, 2,092 deselected (18,474 total collected+deselected) . Moved by two on 2026-09-03 by UI-IMP-WP11: `tests/unit/test_intelligence_artifact_pipeline.py` gained the `structured_content` round-trip and list-cursor/foreign-cursor cases, both FAST-eligible and unmarked. Re-measured rather than assumed. Moved again on 2026-09-03 by the UI-IMP-WP05/WP-06 integration (`origin/main` at `1055e5bc`): `origin/main` recorded 15,316 collected and 2,025 deselected of 17,341, true of *its* tree, and this branch recorded 16,378 / 2,092 of 18,470, true of *its* — neither is true of the merge. Re-measured on the merged tree with every conflict marker gone, and again after this row had itself been rewritten -- which was not a formality: the first reading was 16,379/18,471, taken while the figures were still an unfillable placeholder that `TIER_CLAIM` could not match. Filling them made the claim bindable, which parametrizes one verification test into existence and moved the selection to 16,380/18,472. The stated figure is the one true of the tree that contains the sentence stating it. measured by running it, tree-wide, not by a directory list. **CI reports 16,245 passed and 2 skipped for this same selection, and the difference is fully accounted for**: `tests/architecture/test_wp16_mail_adapter.py` reads Apple's own `Mail.sdef` at a fixed path under `/System`, and its two tests that do so skip when that file is absent. It is present on a macOS worktree and absent on a Linux runner, so the pair runs here and skips there. `claimed + skipped == collected` holds for both readings, which is the identity the tier rule checks. The row previously said 0 skipped while CI said 2 and 3 were failing; the failures are fixed and the skip divergence is now explained rather than unstated. Moved by two on 2026-09-02 under `RULING-M12`, one test at a time and each measured rather than assumed. `test_every_governed_capability_is_granted_or_deliberately_withheld` was added to `tests/contract/test_relationship_intelligence_profiles.py`, a FAST-eligible directory inside the gate-safe set, taking this figure 16,245 -> 16,246. Binding the database-tier cell below then took `TIER_CLAIM` from two matches to three, and that rule parametrizes one verification test per match, so it added one more here and one to the architecture row above: 16,246 -> 16,247 and 4,845 -> 4,846. The deselected figure moves for neither, because both new tests are unmarked. The architecture and FAST figures are the two this document states about the tier that contains the rule reading them, which is why binding the cell was done in its own commit and these were corrected only afterwards. **Moved again on 2026-09-02 by the UI-IMP-WP03 integration** (`origin/main` at `20638373`): collected 16,247 -> 16,297, deselected 2,086 -> 2,090, total 18,333 -> 18,387, and the architecture row 4,846 -> 4,892. Neither this branch's figures nor `origin/main`'s are true of the merged tree -- `origin/main` stated 15,247 collected, 2,023 deselected and 4,842 architecture, each true of its own tree -- and the merged pair is neither side's and is not their sum. Every figure here was re-measured by `--collect-only` on the merged tree with every conflict marker gone, and measured a second time after this row had itself been rewritten, because the row is inside the tier it counts. **Moved again on 2026-09-03 by the entity-plane prose guard**, which adds `tests/architecture/test_entity_plane_prose_matches_the_capability_sets.py` and its 11 tests: collected 16,297 -> 16,308, total 18,387 -> 18,398, architecture 4,892 -> 4,903, and the test-module count 446 -> 447. The deselected figure and the database tier do not move, because the new tests carry no marker this selection excludes. The delta is exactly 11 in all three collections, which is what isolates it: the four prose corrections that commit also makes are comment text and move nothing, and that was measured rather than assumed. **Moved again on 2026-09-03 by the UI-IMP-WP04 integration** (`origin/main` at `cac110ad`): collected 16,308 -> 16,375, deselected 2,090 -> 2,092, total 18,398 -> 18,467, architecture 4,903 -> 4,929, and the `mypy` corpus 447 -> 449. **The `mypy` figure is this integration's FINDING-M1 catch**: this branch and `origin/main` both said 447, from different baselines and for different reasons, so the conflict resolved to identical text and the number would have stood unexamined. It is 449 on the merged tree. Re-measured, never summed. **Moved once more on 2026-09-03 by the entity-plane prose guard's corpus arm**, which adds three tests: collected 16,375 -> 16,378, total 18,467 -> 18,470, architecture 4,929 -> 4,932. The delta is exactly three in all three collections, which isolates it -- the eight prose corrections in the same commit are comment and documentation text and move nothing, measured rather than assumed. `mypy` 449, `src` 318, test modules 449 and the database tier 2,081 are all unmoved: the arm extends an existing module rather than adding one, and carries no marker this selection excludes. **The deselected and total figures moved by exactly two on 2026-09-02 when this branch integrated `origin/main` at `6db2a203` (RI-ENT-WP-09, PR #175), and the collected figure did not.** That merge adds no module and no FAST-eligible test; it adds two `database`-marked tests to `tests/database/test_entity_search_reaches_context.py`, which this selection deselects. Both figures were re-measured by `--collect-only` on the merged tree with every conflict marker gone, and the pair is recorded here because `2,084` auto-merged into this row without a conflict and would have stood wrong: the tier rule reads the bolded pass count and explicitly does not read `deselected`, so nothing would have caught it (FINDING-M1). **Corrected 2026-09-02 by the base merge, by re-measurement of the merged tree rather than by picking a side or by summing the two branches' deltas (RULING-M2).** Both branches corrected this row on 2026-09-02, from different baselines, and neither figure is true of a tree carrying both -- the two corrections do not compose, because the branches added test modules independently and four of them are the same four files. Re-measured with `--collect-only` on the merged tree only after every conflict marker was gone -- a figure read while they are still present carries both branches' claims and parametrizes one verification test per side, and is void -- and re-measured a second time after this row had itself been rewritten, because the row is self-referential. **UI-IMP-WP02's correction of this same row, preserved:** Corrected 2026-09-02 from 14,984 to 15,047, and deselected from 1,974 to 1,987: reconciling UI-IMP-WP02 onto current main added ten FAST-eligible tests (seven unit digest/session-authority cases and three schema declaration/chain checks) plus fifty-three architecture parametrizations over the five new identity-plane modules, which this selection also admits, and thirteen `database`-marked tests this selection deselects. 10 + 53 = 63, which is the whole of the collected movement; 13 is the whole of the deselected movement. This correction states no new per-file test-count claim in the bound form, so it adds no self-referential verification test. Measured directly by `--collect-only` against this exact reconciled tree (15,047/17,034 tests collected, 1,987 deselected). **RI-ENT-WP-10/11's correction of this same row, preserved:** Corrected 2026-09-02 from 16,029 to 16,182, and deselected from 2,030 to 2,036, when RI-ENT-WP-11 published the person-organization-affiliation family, the fifth and last of the five: the collected movement is that family's three mutation names expanding the sweeps that parametrize over `Capability`, and the deselected movement is the six `database`-marked tests it adds to `tests/database/test_entity_family_write_ledger.py`, which have never been executed. Measured directly by `--collect-only` against this exact tree. The correction it supersedes read: corrected 2026-09-02 from 15,876 to 16,029, and deselected from 2,024 to 2,030, when RI-ENT-WP-11 published the project-participation family, the fourth of the five: the collected movement is that family's three mutation names expanding the sweeps that parametrize over `Capability`, and the deselected movement is the six `database`-marked tests it adds to `tests/database/test_entity_family_write_ledger.py`, which have never been executed. Measured directly by `--collect-only` against this exact tree. The correction it supersedes read: corrected 2026-09-02 from 15,723 to 15,876, when RI-ENT-WP-11 published the communication-method family, the third of the five: the movement is that family's three mutation names expanding the sweeps that parametrize over `Capability`, plus this document's own restated per-file claim for `tests/security/test_entity_privacy_regression.py`. Measured directly by `--collect-only` against this exact tree, never by adding an assumed delta. The earlier correction it supersedes read: corrected 2026-09-01 from 15,570 to 15,723, deselected unchanged at 2,004, when RI-ENT-WP-11 published the record families' first mutation family; the earlier correction it supersedes read: corrected 2026-09-01 from 15,134 to 15,403, deselected unchanged at 2,004: this tree now also carries RI-ENT-WP-10 (the six Entity-bound record families' read surface — `entities.profile` and the `entities.*.list` names). The collected figure's +269 is this work package's added capability names expanding the sweeps that parametrize over `Capability`, plus the twenty-one tests in the one module this work package adds, `tests/contract/test_entity_record_family_reads.py`. The deselected figure does not move: this work package adds no `database`-marked test, and the SQL half of its four `*_page` reads is therefore **unexecuted**. **This correction states no new per-file `` `tests/….py` — N tests `` claim**, so it adds no self-referentially parametrized verification test to this selection or to the architecture tier. Measured directly by `--collect-only` against this exact tree (15,403/17,407 tests collected, 2,004 deselected). Prior correction 2026-09-01 from 14,984 to 15,134, and deselected from 1,974 to 2,004: this tree now also carries RI-ENT-WP-09 (entity resolution/search vNext). The movement in both figures is attributed in full, by name, with no residual. Of the collected figure's +150: `bb3bf22` added `tests/unit/test_entity_resolution_vocabulary.py` (+75), `2efd2fc` added thirty-seven tests to the existing `tests/unit/test_entity_resolution.py` (+37), `1062251` added `tests/unit/test_entity_search_reaches_context.py` (+23), and `5700c37` added `tests/contract/test_entity_search_disambiguators.py` and `tests/unit/test_entity_search_disambiguators.py` (+15 between them). 75 + 37 + 23 + 15 = 150. Of the deselected figure's +30: `8f6c576` added `tests/database/test_entity_resolution_value_reads.py` and `1062251`/`5700c37` added and extended `tests/database/test_entity_search_reaches_context.py`, all `database`-marked and so excluded here; **none of the three has ever been executed** — the database tier was closed for the whole of RI-ENT-WP-09 because the WP-08 database gate was running on the same machine, and their figures are collection, not execution. `0571b3e`, `dcae6bd`, `c0e36d7` and `2534a22` changed ports, doubles and evaluation fixtures without adding or removing a collected test, so none of the four moves either figure. **This correction states no new per-file `` `tests/….py` — N tests `` claim**, so it adds no self-referentially parametrized verification test to this selection or to the architecture tier, and the architecture figure below is unmoved at 4,743 — measured, not assumed. Measured directly by `--collect-only` against this exact tree (15,134/17,138 tests collected, 2,004 deselected). Prior correction 2026-09-01 from 14,941 to 14,984, and deselected from 1,956 to 1,974: a corrective cycle closing the independent reviewer's finding D1 landed four commits on this branch — `33d5424` replaced `EntityRecordFamilyService`'s blanket preferred-correction refusal with the three-statement supersession ordering the accepted DDL already admits, `a3b897b` withdrew the "structurally inexpressible" claim from the campaign record, `0e17e91` moved the unit tier onto the corrected behaviour, and `f2db56d` gave every family's correction database-tier coverage. Two of the four move a count, and the movement in both figures is attributed in full, by name, with no residual. `0e17e91` rewrote thirteen unit tests in place and added forty beside them: `tests/unit/test_entity_record_family_service.py` — 185 tests, up from the 145 the correction below measured, so +40 FAST-eligible, and that is the whole of this figure's movement that is not this edit's own. `f2db56d` moved the two `database`-marked modules this selection deselects rather than collects: `tests/database/test_entity_record_family_write_path.py` — 60 tests, up from 45, and `tests/database/test_entity_record_family_service_write_path.py` — 22 tests, up from 19, so +18, which is the whole of the deselected figure's movement. `33d5424` and `a3b897b` changed a service, a port, a repository, `tests/conftest.py` and a campaign document without adding or removing a collected test, so neither moves either figure. **Unlike the correction immediately below, this one does state new per-file `` `tests/….py` — N tests `` claims — three of them, deliberately.** The reviewer's non-blocking finding was that the three RI-ENT-WP-08 per-file figures were written count-first — the count, then the backticked path — and so matched nothing in `tests/architecture/test_claimed_test_counts_match_collection.py`'s `CLAIM` pattern, which requires the backticked path *before* the count and admits at most four separator characters between them. The three figures were correct and bound to nothing. They are restated here in the form the guard reads, so collection now checks all three rather than a reader having to; the count-first figures below are left standing as the history of that correction and are explicitly the figures those modules held at *that* head, not claims about this tree. Each newly bound claim is dynamically parametrized into `test_every_claimed_test_count_matches_collection` at collection time, which adds exactly one FAST-eligible test to this very selection and one to the architecture tier — the same self-referential effect the corrections further below record, three times over: +3 here, and +3 in the Architecture tier row below. 40 + 3 = 43, which is the whole of this figure's movement. The pre-existing count-first per-file figures belonging to RI-ENT-WP-02 through RI-ENT-WP-07 are likewise unbound by `CLAIM`; they are out of this cycle's scope and are left exactly as they stand. Measured directly by `--collect-only` against this exact tree *after* the edit that states these three claims rather than before it, and never by adding an assumed delta (14,984/16,958 tests collected, 1,974 deselected), stable across two consecutive collections. Prior correction 2026-09-01 from 14,782 to 14,941, and deselected from 1,876 to 1,956: this tree now also carries RI-ENT-WP-08 (the six Entity-bound record families' application write path). The movement in the collected figure is attributed in full: 145 FAST-eligible tests in the new module `tests/unit/test_entity_record_family_service.py` as it stood at that head (added by `31cc7bf`, extended by `95b16cf`; it holds 185 now, restated in bound form in the correction above), plus exactly 14 more inside `tests/architecture/`, which this selection also admits — five architecture modules parametrize one case per module under `src/my_pa`, and `1b2dd18` added one such module, `src/my_pa/application/entity_record_families.py`, so 14 ids naming it appear; those 14 are the whole of the architecture row's own movement below. 145 + 14 = 159, which is the whole of this figure's movement. **This correction states no new per-file `` `tests/….py` — N tests `` claim**, so, unlike every correction below, it adds no self-referentially parametrized verification test to this selection or to the architecture tier: the figures here are what the tree collects both before and after this edit. Of the deselected movement, 64 are attributed by name -- 45 in `tests/database/test_entity_record_family_write_path.py` (added by `ed6e057`) and 19 in `tests/database/test_entity_record_family_service_write_path.py` (added by `499a7c1`, extended by `95b16cf`), both `database`-marked and so excluded here; both are the figures those modules held at that head, and both are restated for this tree, in bound form, in the correction above. The remaining 16 was not independently attributed to a specific test id before this correction was written, per the standing instruction that these figures are recomputed from real collection output and never guessed rather than reconstructed from an assumed delta; `87e1e0f` rewrote two `database`-marked modules after the previous pair was measured, and nothing beyond that observation is asserted about the residual. `34367b4` changed source contents without adding a module, so it moves neither figure. Measured directly by `--collect-only` against this exact tree (14,941/16,897 tests collected, 1,956 deselected), stable across two consecutive collections. Prior correction 2026-08-31 from 14,749 to 14,782, and deselected from 1,823 to 1,876: this tree now also carries RI-ENT-WP-07 (fact-level assertion/provenance binding for the six WP-02-WP-06 record families, migration `1cda4d536268`, additive on `9a3f6c1e8d24`) -- RI-ENT-WP-07 added one FAST-eligible module (`tests/unit/test_entity_assertion_domain.py`, 26 tests), one mixed module with two FAST-eligible chain-position/declaration checks plus sixteen `database`-marked tests (`tests/schema/test_entity_assertion_provenance_migration.py`), and one `database`-marked module this selection excludes (`tests/database/test_entity_assertion_provenance.py`, 12). This figure is itself one higher than that: stating this row's own per-file test-count claim above is itself a claim `tests/architecture/test_claimed_test_counts_match_collection.py` dynamically parametrizes into a verification test at collection time, which self-referentially adds exactly one FAST-eligible test to this very selection -- the same effect prior corrections below record, and the Architecture tier row below records the matching effect on that figure. Measured directly by `--collect-only` against this exact combined tree (14,782/16,658 tests collected, 1,876 deselected) rather than attributed by assumption; the remaining movement in both figures beyond RI-ENT-WP-07's own 56 new tests plus this self-referential one was not independently attributed to a specific test id before this correction was written, per the standing instruction that these figures are recomputed from real collection/execution output and never guessed rather than reconstructed from an assumed delta. Prior correction 2026-08-31 from 14,686 to 14,749, and deselected from 1,751 to 1,823: this tree now carries BOTH RI-ENT-WP-06a (the relationship-type taxonomy, revision `8dc3619891bb`) and RI-ENT-WP-06b (merge/split wiring for the six deferred families, revision `9a3f6c1e8d24`, additive on `8dc3619891bb`) -- RI-ENT-WP-06a added one FAST-eligible module (`tests/unit/test_relationship_type_taxonomy_domain.py`, 24 tests) and one mixed module with three FAST-eligible chain-position/declaration checks plus fifty-three `database`-marked tests (`tests/schema/test_entity_relationship_types_migration.py`), and one `database`-marked module this selection excludes (`tests/database/test_entity_relationship_types_tbr_fixture.py`, 1); RI-ENT-WP-06b added two new database-tier test modules this selection excludes (`tests/database/test_ri_ent_wp06b_merge_split.py`, 16; `tests/schema/test_widen_identity_family_vocabulary_migration.py`, 2) and 34 new FAST-eligible tests across two existing modules (`tests/unit/test_identity_correction_planning.py`, 30 new; `tests/unit/test_identity_correction.py`, 4 new). Measured directly by `--collect-only` against this exact combined tree (14,749/16,572 tests collected, 1,823 deselected) rather than attributed by assumption. Prior correction 2026-08-30 from 14,658 to 14,686, and deselected from 1,725 to 1,751: RI-ENT-WP-05 added three new test modules -- one FAST-eligible (`tests/unit/test_person_organization_affiliation_domain.py`, 23 tests) and two `database`-marked modules this selection excludes (`tests/schema/test_person_organization_affiliations_migration.py`, 26; `tests/database/test_person_organization_affiliations_tbr_fixture.py`, 2). This figure is itself one higher than that: stating this row's own per-file test-count claim above is itself a claim `tests/architecture/test_claimed_test_counts_match_collection.py` dynamically parametrizes into a verification test at collection time, which self-referentially adds exactly one FAST-eligible test to this very selection -- the same effect the RI-ENT-WP-04 correction below recorded on this figure, and the Architecture tier row below records the matching effect on that figure. Measured directly by `--collect-only` against this exact tree rather than attributed by assumption. Prior correction 2026-08-30 from 14,571 to 14,658, and deselected from 1,676 to 1,725: measured directly by `--collect-only` and then a full synchronous run against this exact tree (**14,658 passed** with zero failures once `tests/unit/test_identifiers.py::test_contract_prefixes_are_stable` was updated to admit `eppt` — see below), rather than attributed by assumption. This figure is itself one higher than the first measurement this correction made (14,657): stating this row's own per-file test-count claim below is *itself* a claim `tests/architecture/test_claimed_test_counts_match_collection.py` dynamically parametrizes into a verification test at collection time, which self-referentially adds exactly one FAST-eligible test to this very selection -- see the Architecture tier row below for the same effect on that figure. RI-ENT-WP-04 added four new FAST-eligible test modules (`tests/unit/test_project_entity_participation_domain.py`, 84 tests) and three `database`-marked modules this selection excludes (`tests/schema/test_entity_project_participations_migration.py`, 48; `tests/database/test_project_entity_participation_isolation.py`, 2; `tests/database/test_project_participation_synthetic_multi_project_fixture.py`, 1); the remaining movement in both figures was not independently attributed to a specific test id before this correction was written, per the standing instruction that these figures are recomputed from real collection/execution output and never guessed rather than reconstructed from an assumed delta. Prior correction 2026-08-30 from 14,569 to 14,571, deselected unchanged at 1,676: measured directly by `--collect-only` against that tree, diffed test-id-for-test-id against the pre-RI-ENT-WP-04 tree (`eeb3a7d`) in an isolated worktree rather than attributed by assumption. RI-ENT-WP-04 added `IdKind.ENTITY_PROJECT_PARTICIPATION` (`"eppt"`), which is auto-parametrized over every `IdKind` member by `tests/unit/test_identifiers.py::test_every_kind_round_trips` and `tests/unit/test_source_registry.py::test_every_kind_is_issuable_and_well_formed`, adding exactly one FAST-eligible case in each — the only two new test ids that isolated-worktree diff found before this table's own new test modules above were authored; `tests/contract/test_entity_evidence_scope.py`'s three parametrize ids also differ between the two collections but each run regenerates a random opaque suffix for them, so that is cosmetic churn, not a count change. Corrected 2026-08-30 from 14,461 to 14,569, and deselected from 1,623 to 1,676: measured directly by `--collect-only` against this exact tree rather than attributed test-by-test. RI-ENT-WP-03 added 102 FAST-eligible tests in `tests/unit/test_entity_address_and_communication_method_domain.py` and 55 `database`-marked tests (`tests/schema/test_entity_addresses_and_communication_methods_migration.py`, 54; `tests/database/test_entity_addresses_and_communication_tbr_fixture.py`, 1), which this selection excludes; the remaining movement in both figures was not independently attributed to a specific commit before this correction was written, per the standing instruction that these figures are recomputed from real collection output and never guessed rather than reconstructed from an assumed delta. Corrected 2026-08-30 from 14,429 to 14,461, and deselected from 1,601 to 1,623: RI-ENT-WP-02 added 28 FAST-eligible tests in `tests/unit/test_entity_name_and_organization_profile_domain.py`; the remaining movement in both figures was likewise not independently attributed. Corrected 2026-08-29 from 14,427 to 14,429: WP-01's family-coverage closure (RI-P2-BLK-001) added two tests beside the ones already counted here. The deselected count moved separately, from 1,597 to 1,601, when WP-01's discovery-side closure (`ef54106`) added four `database`-marked tests, which this selection excludes -- the passed figure this row's own equality checks is unaffected, since `deselected` sits outside `claimed + skipped == collected`. Earlier that same day it read 14,243, measured on the final proposal-token corrective tree before the RI remediation campaign added tests on both sides; the run that found that stale figure also found `tests/jobs/test_extraction_executor.py::test_the_executor_is_the_handler_the_worker_process_wires` failing on `apps.worker._PLANES`, since renamed to `_JOB_PLANES` for the plane-to-handler pairing — the test has been realigned to the renamed symbol and is green in this figure. A full re-run against the head this correction is bound to reported `1 failed, 14,428 passed, 1,597 deselected in 585.60s`, the one failure being this row's own claim before this edit; the count is green once corrected. |
| Architecture tier | Architecture tier **4,959 passed**, 0 failed. **Moved on 2026-09-03 by the base integration of `origin/main` into `ri-ent/wp13-fixture`, in two steps: `8f0e4779` (PR #177, RI-ENT-WP-10/11, at `e004942b`, and PR #182, UI-IMP-WP07 through WP10, on top of it) and then `1bb7c3cf` (PR #183, UI-IMP-WP11)**: `origin/main` recorded 4,932 collected for this tier, true of *its* tree; this branch recorded 4,896, true of *its* — neither is true of the merge, which carries both branches' guard modules and both branches' per-file claims. Re-measured by `--collect-only tests/architecture` on the merged tree with every conflict marker gone, and again after this row had itself been rewritten, because the row is inside the tier it counts; never by summing the two branches' deltas (RULING-M2). Executed in full at this head in a real checkout at `/Users/bobbyfetting/my-pa-wp13`, whose `ROOT` is not under `.claude`, and the tier reported 0 failed — so the sandbox caveat preserved below from both histories does not apply to this measurement either. Both branches' histories of this row are preserved below, this branch's first and then `origin/main`'s, each true of the tree that wrote it. **This branch's (RI-ENT-WP-13) history of this row, as it stood at `e94dd4b3`, preserved:** Corrected 2026-09-02 at RI-ENT-WP-13 closeout from 4,798 to 4,823: the closeout adds one guard module to this tier holding 24 tests, and the FAST row above states one new per-file `` `tests/….py` — N tests `` claim, which `tests/architecture/test_claimed_test_counts_match_collection.py` — itself a member of this tier — dynamically parametrizes into one further verification test, so 24 + 1 = 25 is the whole of this figure's movement, with no residual. Measured by `--collect-only` against this exact tree *after* the edit that states that claim, stable across two consecutive collections, and executed in full at this head rather than inferred from collection. **The sandbox caveat that follows does not apply to this measurement**: the closeout ran in a real checkout at `/Users/bobbyfetting/my-pa-wp13`, whose `ROOT` is not under `.claude`, and the tier reported 0 failed with the two tests in question passing. The caveat is kept because it explains the 4,798 figure recorded before it — (2 of them fail only in this local sandboxed worktree, not in CI) (`tests/architecture/test_citations_resolve_at_head.py`'s two tests; this sandboxed local worktree's `ROOT` resolves under `.claude/worktrees/agent-.../`, and `.claude` is unconditionally in that test's own `SKIPPED_DIRECTORIES`, so every file under `ROOT` is filtered out 100% reproducibly regardless of code changes -- confirmed by running the identical test file against a real, non-worktree checkout, where it passes cleanly; CI runs in a real clean checkout and is the authoritative signal for this file) on the RI remediation tree, 4,743 collected. This path-selected figure is stated separately so its checked claim cannot be mistaken for the FAST marker result. **`origin/main`'s (RI-ENT-WP-10/11) history of this same row, as it stood at `e004942b`, preserved -- its lead figure is restated in prose rather than in the bound form, so that the path rule binds only the figure at the head of this cell:** `origin/main`'s tree collected 4,932 for this tier, all passing there (2 of them fail only in this local sandboxed worktree, not in CI) (`tests/architecture/test_citations_resolve_at_head.py`'s two tests; this sandboxed local worktree's `ROOT` resolves under `.claude/worktrees/agent-.../`, and `.claude` is unconditionally in that test's own `SKIPPED_DIRECTORIES`, so every file under `ROOT` is filtered out 100% reproducibly regardless of code changes -- confirmed by running the identical test file against a real, non-worktree checkout, where it passes cleanly; CI runs in a real clean checkout and is the authoritative signal for this file) on the merged tree, 4,932 collected. This path-selected figure is stated separately so its checked claim cannot be mistaken for the FAST marker result. **Corrected 2026-09-02 by the base merge, by re-measurement of the merged tree rather than by picking a side or by summing the two branches' deltas (RULING-M2).** Both branches corrected this row on 2026-09-02, from different baselines, and neither figure is true of a tree carrying both -- the two corrections do not compose, because the branches added test modules independently and four of them are the same four files. Re-measured with `--collect-only` on the merged tree only after every conflict marker was gone -- a figure read while they are still present carries both branches' claims and parametrizes one verification test per side, and is void -- and re-measured a second time after this row had itself been rewritten, because the row is self-referential. **UI-IMP-WP02's correction of this same row, preserved:** Corrected 2026-09-02 from 4,743 to 4,796: UI-IMP-WP02 added five identity-plane modules under `src/my_pa`, and five modules of this tier parametrize per source module, producing fifty-three new ids over those five files. That is the whole of this figure's movement. This correction states no new per-file test-count claim, so the claimed-count guard adds nothing. Measured directly by `--collect-only` of `tests/architecture` against this exact reconciled tree. **RI-ENT-WP-10/11's correction of this same row, preserved:** Corrected 2026-09-02 from 4,786 to 4,792: RI-ENT-WP-11's three person-organization-affiliation names expand the architecture-tier sweeps that parametrize over `Capability`, and that is the whole of this figure's movement. Measured directly by `--collect-only tests/architecture` against this exact tree. Prior correction 2026-09-02 from 4,780 to 4,786: RI-ENT-WP-11's three project-participation names expand the architecture-tier sweeps that parametrize over `Capability`, and that is the whole of this figure's movement. Measured directly by `--collect-only tests/architecture` against this exact tree. Prior correction 2026-09-02 from 4,774 to 4,780: RI-ENT-WP-11's three communication-method capability names expand the architecture-tier sweeps that parametrize over `Capability`, and that is the whole of this figure's movement. Measured directly by `--collect-only tests/architecture` against this exact tree. Prior correction 2026-09-01 from 4,743 to 4,748: RI-ENT-WP-10's added capability names expand the architecture-tier sweeps that parametrize over `Capability`, and that is the whole of this figure's movement — the work package states no new per-file `` `tests/….py` — N tests `` claim, so it adds no dynamically parametrized verification test to this tier. Measured directly by `--collect-only tests/architecture` against this exact tree. Prior correction 2026-09-01 from 4,740 to 4,743: the FAST tier row above now states three new per-file test-count claims — the three RI-ENT-WP-08 modules, restated in the bound `` `tests/….py` — N tests `` form after the independent reviewer found them written count-first and therefore matched by nothing — and `tests/architecture/test_claimed_test_counts_match_collection.py`, itself a member of this tier, dynamically parametrizes one verification test per such claim found in this document at collection time, so those three claims add exactly three tests to this tier's own collected count. That is the whole of this figure's movement: the corrective cycle's four commits (`33d5424`, `a3b897b`, `0e17e91`, `f2db56d`) added no module under `src/my_pa` and no test under `tests/architecture/`, so the five per-source-module parametrizations named below are unmoved, and there is no residual. Measured directly by `--collect-only` against this exact tree *after* the edit that states those three claims, stable across two consecutive collections. Prior correction 2026-09-01 from 4,726 to 4,740: `1b2dd18` added one module under `src/my_pa` -- `src/my_pa/application/entity_record_families.py`, RI-ENT-WP-08's application service -- and five modules of this tier parametrize per source module: `tests/architecture/test_dependency_direction.py` (8 ids), `tests/architecture/test_principal_is_never_caller_supplied.py` (2 ids), `tests/architecture/test_scope_and_hygiene.py` (2 ids), `tests/architecture/test_async_stays_at_the_transport_edge.py` (1 id) and `tests/architecture/test_principal_partition_is_reached_through_the_guard.py` (1 id), so one new source module adds exactly 14, every one of them naming `entity_record_families.py` in its id. That accounts for this figure's movement in full. **Unlike the corrections below, this one is not self-referential**: the FAST tier row above states no new per-file test-count claim, so `tests/architecture/test_claimed_test_counts_match_collection.py` -- itself a member of this tier -- parametrizes no additional verification test and adds nothing to this count. `28fb1e5` added six entries to `VERIFIED_CALLER_STATEMENTS` in `tests/architecture/test_principal_is_never_caller_supplied.py`, but those feed a measurement inside an existing test rather than a parametrization and add no collected test; the 14 above account for the movement exactly, with no residual. Measured directly by `--collect-only` against this exact tree, stable across two consecutive collections. Prior correction 2026-08-31 from 4,725 to 4,726: the FAST tier row above states RI-ENT-WP-07's own new per-file test-count claims, and `tests/architecture/test_claimed_test_counts_match_collection.py` -- itself a member of `tests/architecture/` -- dynamically parametrizes one verification test per such claim found in this document at collection time, so that adds exactly one test to this tier's own collected count too. Measured directly by `--collect-only` against this exact tree. Prior correction 2026-08-31 from 4,724 to 4,725: this tree now carries both RI-ENT-WP-06a and RI-ENT-WP-06b, each of which states its own new per-file test-count claim above, and `tests/architecture/test_claimed_test_counts_match_collection.py` -- itself a member of `tests/architecture/` -- dynamically parametrizes one verification test per such claim found in this document at collection time, so those two claims add exactly two tests to this tier's own collected count too. Measured directly by `--collect-only` against this exact tree. Prior correction 2026-08-30 from 4,723 to 4,724: the FAST tier row above states RI-ENT-WP-05's own new per-file test-count claim, and `tests/architecture/test_claimed_test_counts_match_collection.py` -- itself a member of `tests/architecture/` -- dynamically parametrizes one verification test per such claim found in this document at collection time, so that single claim adds exactly one test to this tier's own collected count too. Measured directly by `--collect-only` against this exact tree. Prior correction 2026-08-30 from 4,722 to 4,723: the FAST tier row above states one new per-file test-count claim, and `tests/architecture/test_claimed_test_counts_match_collection.py` -- itself a member of `tests/architecture/` -- dynamically parametrizes one verification test per such claim found in this document at collection time, so that single claim adds exactly one test to this tier's own collected count too. Measured directly by `--collect-only` against this exact tree, stable across two consecutive collections. Corrected 2026-08-29 from 4,712, which was measured on the integrated corrective tree before this campaign's architecture tests were added. |
...
| Types | `mypy` (configured targets: `src`, `migrations`, `apps`, `ops`) — clean over 449 files: `Success: no issues found in 449 source files`. **Moved on 2026-09-03 by the base integration of `origin/main` into `ri-ent/wp13-fixture`, in two steps: `8f0e4779` (PR #177, RI-ENT-WP-10/11, at `e004942b`, and PR #182, UI-IMP-WP07 through WP10, on top of it) and then `1bb7c3cf` (PR #183, UI-IMP-WP11)**: `origin/main` recorded 449, true of *its* tree, and this branch recorded 447, true of *its*. Measured by running the type check over the configured targets on the merged tree with every conflict marker gone, and cross-checked against the declaration the tier guard derives from; RI-ENT-WP-13 adds no file under any configured target, so the merged corpus is `origin/main`'s — which was measured rather than relied on (RULING-M2). Both branches' histories of this row are preserved below, this branch's first and then `origin/main`'s. **This branch's (RI-ENT-WP-13) history of this row, as it stood at `e94dd4b3`, preserved:** Corrected 2026-09-03 from 441 to 447 at the RI-ENT-WP-13 integration of `1055e5bc`, which brought six typed modules (two HTTP adapters, two application modules, one domain module, one security-infrastructure module for the WebAuthn and auth-session work) — the tier guard reddened on 441 before this sentence changed, which is the guard working. Corrected 2026-09-02 from 435 to 441: UI-IMP-WP02 added four domain modules, one persistence module, and one Alembic revision under the configured `src`/`migrations` targets. Measured by running `mypy` against this exact reconciled tree. **`origin/main`'s (RI-ENT-WP-10/11) history of this same row, as it stood at `e004942b`, preserved -- its lead figure is restated in prose rather than in the bound form, so that the tool rule binds only the figure at the head of this cell:** the type check on `origin/main`'s tree reported success over 449 source files. **Corrected 2026-09-02 by the base merge, by running the type check over the configured targets on the merged tree rather than by summing the two branches' corrections (RULING-M2).** **UI-IMP-WP02's correction of this same row, preserved:** Corrected 2026-09-02 from 435 to 441: UI-IMP-WP02 added four domain modules, one persistence module, and one Alembic revision under the configured `src`/`migrations` targets. Measured by running `mypy` against this exact reconciled tree. **RI-ENT-WP-10/11's correction of this same row, preserved:** Corrected 2026-09-02 from 436 to 437: this tree now also carries the phase's single migration `migrations/versions/20260902_16f05c46b8c3_admit_the_ri_ent_wp_10_11_entity_.py` (`959f6c1b`), under the configured `migrations` target -- the only file added under any of the four configured targets since the previous correction, since `bcd2048` added a test module, which no configured target covers. Measured by running the type check over the configured targets on this exact tree, which reported `Success: no issues found in 437 source files`, and cross-checked against the declaration the guard derives from; the two agree at 437. Prior correction 2026-09-01 from 434 to 435: this tree now also carries RI-ENT-WP-08's application service `src/my_pa/application/entity_record_families.py` (`1b2dd18`), under the configured `src` target — the only file added under any of the four configured targets since the previous correction, since `31cc7bf` and `499a7c1` added test modules, which no configured target covers, and `95b16cf` and `34367b4` changed existing files without adding one. Measured by running the type check over the configured targets on this exact tree, which reported `Success: no issues found in 435 source files`, and cross-checked against the declaration the guard derives from; the two agree at 435. Prior correction 2026-08-31 from 433 to 434: this tree now also carries the WP-08 blocker-clearing rename migration `migrations/versions/20260831_c99cd8ed8d1c_rename_design_coordinates_with.py` (renaming the seeded `entity_relationship_types` row `design_coordinates_with` to `design_coordination_with` and closing `EntityRelationshipType` to 35-of-35 parity), under the configured `migrations` target. Prior correction 2026-08-31 from 432 to 433: this tree now also carries RI-ENT-WP-07's migration `migrations/versions/20260831_1cda4d536268_add_entity_assertions_and_evidence.py`, under the configured `migrations` target. Prior correction 2026-08-31 from 430 to 432: this tree now carries both RI-ENT-WP-06a's migration `migrations/versions/20260831_8dc3619891bb_add_entity_relationship_types.py` and RI-ENT-WP-06b's migration `migrations/versions/20260831_9a3f6c1e8d24_widen_identity_family_vocabulary_for_ri_ent_wp06b.py`, both under the configured `migrations` target. Measured directly by running `mypy` against this exact combined tree rather than summed by assumption. Corrected 2026-08-30 from 429 to 430: RI-ENT-WP-05's migration `migrations/versions/20260830_17149a48fa30_add_entity_person_organization_affiliat.py` is under the configured `migrations` target. Corrected 2026-08-30 from 428 to 429: RI-ENT-WP-04's migration `migrations/versions/20260830_f5b06925857e_add_entity_project_participations_and_.py` is under the configured `migrations` target. Corrected 2026-08-30 from 427 to 428: RI-ENT-WP-03's migration `migrations/versions/20260830_441b071bf37b_add_entity_addresses_and_communication_.py` is under the configured `migrations` target. Corrected 2026-08-30 from 426 to 427: RI-ENT-WP-02's migration `migrations/versions/20260830_7e114f822af2_add_entity_names_and_organization_.py` is under the configured `migrations` target. Corrected 2026-08-29 from 424, which was the corpus at the audited base; the RI remediation campaign added source modules on top of it. The figure is bound to `[tool.mypy] files` by `tests/architecture/test_claimed_test_counts_match_collection.py`, which counts the declaration rather than re-running the type check. |
| Full database tier disposition | `pytest -m "database or recovery or e2e"` — this selection collects 2,095 on this tree, so a green run is **2,095 passed**, 0 skipped. **FULL_DB_TIER_EXECUTED_AT_TREE_3115020c, 2026-09-03 -- PENDING RE-EXECUTION ON THIS TREE.** Executed in full, chunked and summed, every invocation exiting 0 and `git write-tree` recorded before and after each and identical every time, at tree `3115020c` -- the working tree of the first base-integration step (`origin/main` at `8f0e4779`, which carries `e004942b`, PR #177, RI-ENT-WP-10/11, and PR #182, UI-IMP-WP07 through WP10) with every conflict marker gone and this cell's census already bound; it became head `903b8b15`, whose committed tree differs from `3115020c` only in this document. **That run does not carry to this tree by identity:** the second step, `origin/main` at `1bb7c3cf` (PR #183, UI-IMP-WP11), changed four modules under `src/` -- `application/commands.py`, `application/intelligence.py`, `application/service.py` and `infrastructure/persistence/intelligence.py` -- and `git diff-tree -r --name-only 903b8b15 <this tree>` shows no database-selected test module, no `conftest.py` on a database-selected path, no migration and no `pyproject.toml`/`uv.lock` among the 37 blobs that differ. The selection's census on this tree is therefore unchanged at 2,095, which is what the bound figure above states; whether it *passes* here is not yet measured, and this cell is re-bound to an execution on this tree before this integration is committed. `tests/database` in three chunks of twenty, twenty and eighteen files, 344 + 388 + 264 = 996; `tests/schema` 662; `tests/migration` 38, run alone because its `conftest.py` and `tests/jobs`'s both import `SCHEMA_VERSION` as a top-level name and cannot share an invocation; then the ten remaining directories each in its own invocation, 434 (security 196, search_quality 84, capture 32, concurrency 23, contract 18, pipeline 15, jobs 14, end_to_end 13, recovery 3, provider_conformance 1). 996 + 662 + 38 + 434 = 2,130 is not the figure -- 996 + 662 + 38 + 196 + 84 + 32 + 23 + 18 + 15 + 14 + 13 + 3 + 1 = 2,095, which equals the `--collect-only` census of this selection at the same tree, directory by directory, so no directory was silently skipped and none double-counted. Fifteen invocations. The whole tree reconciles at that census (re-measured on this tree after the `1bb7c3cf` step): the FAST selection collects 16,425, this selection 2,095, and the eleven tests outside both are enumerated by name -- five `slow` in `tests/contract/test_task_mcp_protocol_harness.py`, two `evaluation` in `tests/evaluation/test_semantic_gate_harness.py`, one `slow` in `tests/integration/test_gsqs_remote_eval_http.py`, three `connector` in `tests/unit/test_wp12_slice_c_application.py` -- and 16,425 + 2,095 + 11 = 18,531, the tree-wide collection, with the two selections measured disjoint. The prior figures were 2,028 on this branch and 2,081 on `origin/main`, each true of its own tree and neither of the merge; re-measured, never summed (RULING-M2). **This branch's (RI-ENT-WP-13) history of this row, as it stood at `e94dd4b3`, preserved -- its lead figure is restated in prose rather than in the bound form, so that the tier rule binds only the figure at the head of this cell:** that row's selection collected 2,028 on this branch's tree, so a green run there was 2,028 passed, 0 skipped; FULL_DB_TIER_EXECUTED_AT_THAT_HEAD, 2026-09-03. Executed in full, chunked and summed: `tests/database` 372 + 345 + 212 = 929, `tests/schema` 662, then the eleven remaining directories 437 (security 196, search_quality 84, migration 38, capture 32, concurrency 23, contract 18, pipeline 15, jobs 14, end_to_end 13, recovery 3, provider_conformance 1). 929 + 662 + 437 = 2,028, which equals the `--collect-only` census at the same head, so no directory was silently skipped and none double-counted. Fifteen invocations, every one exiting 0. **Superseded — the prior disposition read FULL_DB_TIER_NOT_RUN_TARGETED_EVIDENCE_SUFFICIENT, true until this head.** The immutable-origin correction used a separately verified, localhost-only, auto-removed PostgreSQL 17.10 container for 82 focused tests; the proposal-token correction recreated the same bounded target pattern for five unequal-version cases. Neither authorized or ran the full database tier. The `database or recovery or e2e` selection collects **2,028** tests on this tree (2,022 before the base integration of `1055e5bc`, which brought six database-tier tests; the tier was re-executed in full on the merged tree, tree-fingerprint `3d910383`, every chunk exiting 0 -- `3d910383` was a `git write-tree` fingerprint of the working tree that became `e94dd4b3`, not a commit, and an independent reviewer at `e94dd4b3` noted that a cell citing a tree no reader can check out is weaker evidence than one citing the committed head; the execution that governs this cell now is the one stated at its head, against the committed tree named there). Corrected 2026-09-02 at RI-ENT-WP-13 closeout from 1,564, which was stale by 458 and had been so for several base integrations. **Why it drifted, which matters more than the figure:** this cell states its selection in prose without a `pytest -m "…"` command and without a bolded pass count, so `tests/architecture/test_claimed_test_counts_match_collection.py`'s tier rule never reads it and nothing reddened while it went stale — the same structural gap that let the FAST *deselected* figure drift, now recorded on a second independent cell. The figure is a `--collect-only` census against this exact tree, and **collection is not execution evidence** — which is why the run above was performed rather than the census cited. It had not been executed when that sentence was written, because the campaign's machine-wide database serialization gate was held by a sibling worktree throughout this closeout (verified, not assumed — `/Users/bobbyfetting/my-pa-wp10` was observed running `tests/database/…` under `/tmp/wp10_gate_db.sh`). Targeted schema, migration, SQL-structure, concurrency, and database execution evidence is recorded in the campaign completion report. **This cell is now bound.** It previously stated its selection without a `pytest -m "…"` command and without a bolded pass count, so `test_claimed_test_counts_match_collection.py`'s tier rule matched nothing and the figure drifted stale by 458 with nothing reddening. Binding it parametrizes exactly one verification test into existence, which moves the FAST and architecture figures above by one each — both re-stated in the same commit and re-measured after it, since a figure must be true of the tree that contains the sentence stating it. **`origin/main`'s (RI-ENT-WP-10/11) history of this same row, as it stood at `e004942b`, preserved -- its one bound figure is restated in prose rather than in the bound form, so that the tier rule binds only the figure at the head of this cell:** **FULL_DB_TIER_NOT_RUN_TARGETED_EVIDENCE_SUFFICIENT.** The immutable-origin correction used a separately verified, localhost-only, auto-removed PostgreSQL 17.10 container for 82 focused tests; the proposal-token correction recreated the same bounded target pattern for five unequal-version cases. Neither authorized or ran the full database tier. The `database or recovery or e2e` selection collects 2,081 tests on this tree, counted by `--collect-only` on the merged tree rather than taken from any branch: RI-ENT-WP-10/11 stated 1,551 and UI-IMP-WP02 stated 1,564, neither is true of a tree carrying both, and neither was true of its own tree either -- this figure was for a long time bound by no guard (the tier rule reads a `pytest -m` command carrying a real marker expression, with a bolded pass count beside it, which this cell did not write) and had drifted accordingly. The command is written here without its expression on purpose: spelled out in full it would be captured as a second claim on this cell, against an expression of literal dots that pytest cannot compile. The 2,081 is 2,078 `database`-marked, 17 `recovery`, 16 `e2e`, overlapping, each measured by its own `--collect-only`; collection is not execution evidence for the unselected tests. **Corrected from 2,073 on 2026-09-02 when this branch integrated `origin/main` at `6db2a203` (RI-ENT-WP-09, PR #175).** That merge adds two `database`-marked tests to `tests/database/test_entity_search_reaches_context.py` -- the widened search's active-state filter, which nothing had held -- and adds no module. `2,073` auto-merged into this cell without a conflict marker and would have stood wrong for exactly the reason this cell already records about itself: nothing reads it. Re-measured, not adjusted by an assumed delta, and re-measured again after this cell had itself been rewritten. **Corrected again on 2026-09-02 by the UI-IMP-WP03 integration** (`origin/main` at `20638373`), from 2,075 to 2,079: that base adds `tests/database/test_webauthn_ceremony.py`, whose tests carry the `database` marker, so this selection genuinely grew. Re-measured on the merged tree, not adjusted by the four this integration appears to add. Targeted schema, migration, SQL-structure, concurrency, and database execution evidence is recorded in the campaign completion report. **Superseded from the RI-ENT-WP-10/11 closeout onward, where this disposition no longer holds: the selection has been executed in full, chunked by directory, at every head that claimed it.** **At `3dfa74f9`: 2,075 passed, 0 failed, 0 errors**, chunks `tests/database` 322 + 393 + 261 = 976, `tests/schema` 662, the eleven remaining directories 437 -- 976 + 662 + 437 = 2,075, the `--collect-only` figure at that head. **At `9c1e463b`, the UI-IMP-WP04 merge and the head this row is stated at: 2,081 passed, 0 failed, 0 errors**, chunks `tests/database` 322 + 393 + 267 = 982, `tests/schema` 662, the eleven directories 437 -- 982 + 662 + 437 = 2,081. Fifteen invocations, every one exiting 0. The tier grew by six across those two heads: four from UI-IMP-WP03's `tests/database/test_webauthn_ceremony.py` and two from UI-IMP-WP04 extending `tests/database/test_webauthn_auth_persistence.py`, traced to their modules rather than inferred from the delta. **The figure and the chunk breakdown are stated against the head each was measured at**, because the previous revision of this sentence carried `3dfa74f9`'s chunks under a later head's total and summed to 2,075 while claiming 2,081 -- in a sentence whose whole load is that the chunks sum exactly. The bolded pass count is guard-read and was dragged forward; the prose beside it was not. One earlier attempt at the eleven ran them in a single invocation, died at collection with five `ImportError`s because `tests/jobs` and `tests/migration` both hold a `conftest.py` that their modules import as a top-level name, and was discarded rather than counted. **Bound to the tier rule on 2026-09-02 under `RULING-M12`, which is what closes the structural gap recorded twice above:** the `pytest -m` command with the expression `database or recovery or e2e` — 2,081 passed, 0 skipped, executed in full and chunked at `3dfa74f9` (written in the bound form on `origin/main`; restated here in prose because this cell's head now carries the binding for the merged tree). Written in the form `test_claimed_test_counts_match_collection.py` actually reads, so this figure is now derived against a collection census instead of resting on a reader noticing it. It had drifted twice before this — 1,551 on both branches when the tree held 2,073, then 2,073 when the merge made it 2,075 — and both times for the same reason, which was never that the number was hard to measure but that nothing was looking. The binding was deliberately not done in the commit that measured these counts, because adding a parametrized verification test changes the architecture and FAST figures this same document states; it is done here alone, and those two figures are re-measured after it. |
...
| Migrations apply and reverse | `tests/schema/test_entity_schema_migration.py` derives the current inventory. Phase B owns seven tables across six revisions (`c7a1f04b9e63`, `d38e6b2fa715`, `e5b0c94d7182`, `a1f7d3c85e40`, `b64e29a0f7c1`, `3d07af4dc513`): the prior five proposal/review/identity tables plus `relationship_write_requests` and its evidence child. The cumulative head revision also carries the B1/B4/B5 column, constraint, backfill and trigger deltas. The merged linear chain admits GSQS at `c4b0a1d9e827` immediately before Phase B begins at `c7a1f04b9e63`; `b727e870d45e` was the single head when this cell was written, additive on `8e1c4a7b2d90`, and carries this campaign's merge-preview ambiguity and settlement tables, the `partial` re-enrichment state with its `limitations` column, the `reenrichment` heartbeat plane and the append-only trigger on `entity_proposal_review_decisions`. Corrected 2026-08-29 from `8e1c4a7b2d90`. Superseded 2026-09-02 at RI-ENT-WP-13 closeout: the chain is now **87 revisions at single head `2c00c9ac64bc`**, measured on this tree by `ls migrations/versions/*.py | wc -l` and `ScriptDirectory.get_heads()`; `b727e870d45e` remains the 78th revision and still carries everything this cell attributes to it, so the attribution stands and only the word "head" was stale. Database execution still requires the disposable PostgreSQL fixture; collection is not execution evidence. |
| Every difficult record the audit names has a typed home | `tests/database/test_tbr_completeness_fixture.py` — 14 tests: one coherent synthetic register, seeded into its own disposable database, carrying all twelve of the source audit's section K difficult-record structures at once — operating/brand identity against owning legal entity with the project and legal-principal addresses as separate typed rows; a single-purpose vehicle that is its own entity rather than an alias of its parent; a current legal identity beside historical juristic predecessors joined by `historical_identity_of`; a legal identity left `unresolved` with zero `legal` name rows rather than a fabricated string; a prior theory carried as a `contradicted` assertion superseded by its correction rather than as an alias treated as true, beside a project-scoped `technical_reviewer_of` edge that is not a corporate affiliation; a time-bounded `acquired_by` window between two connected practices; participation scope carried in `scope_text`; a contracting affiliate left `awaiting_confirmation` with no guessed edge; a held participation whose entity status is not overloaded; a person participating directly with a null `organization_entity_id` and no placeholder company minted for them; a survivor that stays `active` while its lineage is `historical`; and a settled `legal_identity_status_code` beside an unsettled `assertion_status`, which are two columns on two rows and, per RULING-M3, neither of them a confidence. The last two tests are the acceptance criterion: one reads `information_schema` and proves no table this campaign created carries a `json` or `jsonb` column and that the register wrote into neither pre-existing `jsonb` ledger, the other states the exact row count in every family the register exercises so a dropped row reddens. Every name, address, domain, mailbox and phone number is invented and every domain is `example.invalid` (RULING-M6). **This module is committed and NOT executed at this head** — the database tier uses fixed disposable-database names and is strictly serial machine-wide, and three sibling work packages were active; it was statically cross-checked instead, every table, column and closed-vocabulary literal verified against the migration that admits it |
| The security matrix is asserted, not assumed | `tests/unit/test_entity_record_family_security_matrix.py` — 16 tests: the structural half of the source audit's section N, needing no database. Server-issued canonical ids (no `Record*` command lets a caller choose the identifier of a new row, and each family mints under its own registered `IdKind`); no opaque JSON catch-all on any of the eleven tables this campaign created; the partition split, asserted in both directions, between the eight Principal-bearing families and the three deliberately global seeded vocabularies; no delete verb declared on either the published command surface or the repository port, with a vacuity control proving the scan reaches a real population; every transition on these families guarded by an `expected_version`; merge and split enumerating every Entity-bound family, with the assertion plane's exclusion asserted as a *recorded decision* so that adding it later reddens and forces the campaign record to be updated with it; and merge and split reserved to the operator within the entity plane. Where an item was already proven the existing test is cited in the campaign record rather than duplicated, and each citation was checked by reading the test body. The one item it does not assert is the free-text upper bound, because no such bound exists — that is recorded as an open item, not papered over |
| The `project_display_name` boundary is mechanical, not prose | `tests/architecture/test_a_project_name_is_never_a_global_identity.py` — 24 tests: RI-ENT-WP-04 wrote the prohibition into the column's own DDL text and called it “the single most important semantic boundary in this work package”, and it was restated in `persistence.tables`, in `EntityProjectParticipation`'s docstring and in the revision docstring — **all four statements were prose, and nothing reddened if production code violated the rule in either direction** (`D-81`). RI-ENT-WP-12's Orchestrator found this and declined to author the guard, on the ground that a guard written by the context that wants the rule relaxed is not evidence; that reasoning is preserved, and RI-ENT-WP-13 authors it instead because it owns the security matrix and has no backfill in scope. Three rules at expression and statement granularity — module granularity is useless because `infrastructure/persistence/entity.py` names a global identity column thirty-eight times while legitimately owning both write sites — covering the provenance of the project-scoped name, the reverse direction (previously guarded by nothing at all), and executed SQL text, which is the direction an expression rule structurally cannot see and the one a bulk backfill would take. A fourth rule freezes the write-site and read-site populations as **equalities**, so a new site cannot appear silently. Every rule is paired with a plant that asserts it fires and a negative control that asserts it does not fire on the live surface; two false positives — the legacy ported `core.daily_brief_change_events.project_display_name` and the CHECK constraint named after the column — are registered by name and measured rather than matched around. **It is a syntactic rule and its docstring says so**: a value laundered through a local variable, a `**kwargs` splat, a mapping built by a helper, or SQL assembled from non-constant parts all pass it, and closing those needs a reaching-definitions analysis this repository does not have. Bite-proved twice at this head, by the author and independently by the controlling context: planting `project_display_name=entity.canonical_name` at `entity_record_families.py:1485` reddens two of the 24 with the file, line, expression and matched token named; the file was restored byte-for-byte (`shasum` equal) and the guard is green again |
...
| False-resolution rate | [`tests/evaluation/RESOLUTION_CALIBRATION.md`](../../tests/evaluation/RESOLUTION_CALIBRATION.md) — 0 false resolutions, 0 cross-Principal leaks, 0 forbidden candidates, recall 1.0 over **51** labelled collision-biased cases in 26 families (18 must-resolve, 32 must-not). Corrected 2026-09-01 from 35/20/14/20: RI-ENT-WP-09 added sixteen cases and six families, so the resolver's new evidence paths — typed names, communication values, and the affiliation and participation signals — are measured rather than merely reachable. The figures are read from the regenerated record rather than summed by assumption. **This claim is invisible to `tests/architecture/test_claimed_test_counts_match_collection.py`'s `CORPUS_CLAIM` pattern**, whose `[\w-]+` cannot match the bold `**51**`, so it is corrected by hand and is not guarded; the one claim in this document that the guard does read is the corpus-size cell above. Corrected 2026-08-19: this row read 27/12/15, which matched no head — the published record has said 31/14/17 since the currency fix in section 4a. Re-derived rather than restated, by `compute_calibration_record()` itself: `PYTHONPATH=src:. python -c "from tests.evaluation.resolution_harness import compute_calibration_record as c; r=c(); print(r['cases'], r['must_resolve_cases'], r['must_not_resolve_cases'])"` → `35 14 20` |
...
* **The off-switch withheld publication and not execution.**
  `available_capabilities` subtracts the fifty-four `entities.` names, and its two
...
* **The corpus is small and synthetic.** Fifty-one labelled cases over twenty-seven
