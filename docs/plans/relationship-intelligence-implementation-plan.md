...
`status: PROPOSED_SUCCESSOR_READY_FOR_OPERATOR_REVIEW` from its creation to this
head. A specification cannot have been retroactively controlling over work that
predates it, and a proposal awaiting operator review is not controlling over
...
| **WP-RI-05** — MCP read/resolve/context surface with bounded output and pagination | Five read capabilities, each bounded and paginated | Delivered in full for the bounded half, and for one of the two capabilities that need pagination. Five `Capability` members (`ENTITIES_SEARCH/GET/RESOLVE/CONTEXT/RELATIONSHIPS`), the `entity_read` purpose, the composition gate, remote-exposure withholding (`tests/contract/test_entity_remote_exposure.py`). `entities.relationships` is now genuinely paginated: `GetEntityRelationships` takes `page_size` **and `after`**, `SqlEntityRepository.relationships` takes `after_relationship_id` and keysets on `relationship_id >`, and the disclosure issues `next_cursor` only when the page was truncated (`src/my_pa/application/commands.py`, `application/service.py`, `infrastructure/persistence/entity.py`; `tests/contract/test_entity_read_bounds.py`). `SearchEntities` takes a bounded `page_size` | **`entities.search` is the one read left unpaginated.** It has `page_size` and no cursor or offset, so a caller can bound one page of a name search and has no way to ask for the next one. The other three need none: `entities.get` and `entities.context` each answer about one named entity, and `entities.resolve` returns a candidate set that is bounded by design and would be wrong to page through. Corrected 2026-08-19: this row read "**No pagination**: there is no cursor or offset parameter anywhere in the search contract", which was true when written and was left standing after the sibling track landed the `entities.relationships` cursor it told the reader to watch for. Derived by reading the contract: `grep -rn "next_cursor\|after_relationship_id" src/` and the `after` field on `GetEntityRelationships` |
...
| **WP-RI-11** — security, privacy, authorization, adversarial testing | Principal isolation, impersonation rejection, minimal-disclosure, adversarial coverage | Delivered, and substantive. `tests/security/test_entity_privacy_regression.py` (19 tests — corrected 2026-08-19 from 11, by `--collect-only -q` on the file: cross-Principal disclosure denial, name-as-instruction injection resistance, judgement-free answers, task-profile draft/gating checks) and `tests/security/test_cross_principal_relationship_isolation.py` (8 tests: foreign-read-as-absent, cross-Principal merge/decide refusal, caller-supplied-Principal rejection, partition non-mutation). This is legacy `WP-RI-11`, and the two maps agree here | Adversarial coverage for the write paths that do not yet have a caller (WP-RI-06) has nothing to exercise yet; revisit once WP-RI-06's caller exists |
| **WP-RI-12** — program-scale synthetic acceptance and performance: ≥500 persons, ≥100 organizations, several programs/projects/work packages, ≥5,000 combined aliases/identifiers/assignments/relationships/observations, ≥50 collision groups, ≥50 historical assignment changes; p50/p95 benchmarks for exact resolve, candidate search, contextual resolution, entity search, context-card assembly, bounded relationship traversal | A fixture and benchmark record at that floor | **A fixture at the floor now exists and nothing runs it; no benchmark exists at all.** The sibling track this row used to point forward to has landed: `tests/evaluation/fixtures/program_scale_corpus.py` and `program_scale_cases.py` are on disk at this head, and the corpus clears every stated minimum — 748 entities — **565 persons and 105 organizations**, 6 programs, 24 projects, 48 work packages, which sum to 748. Corrected 2026-08-19: this cell said 500 and 100, which are the lengths of the builder's `persons` and `organization_ids` helper collections rather than the entity counts, and sum to 678. It said so in a sentence claiming the figures were "derived by loading it, not by reading its constants" — the one place it was reading constants, 5,262 combined alias/identifier/assignment/relationship/observation records (680 + 910 + 1,250 + 922 + 1,500), 60 collision groups, 80 historical employment changes — with 1,090 labelled cases beside it. Derived by loading it, not by reading its constants: `PYTHONPATH=src:. python -c "from tests.evaluation.fixtures.program_scale_corpus import PROGRAM_SCALE_CORPUS as C; import dataclasses; [print(f.name, len(getattr(C, f.name))) for f in dataclasses.fields(C)]"`. **The honest fact about it is that it is inert.** `grep -rn "program_scale" tests/ src/ scripts/ apps/` returns nothing outside the two fixture files themselves — no test imports either module, no tier executes them, and `tests/evaluation/fixtures/__init__.py` does not export them. A fixture no test reads proves the same amount as no fixture; what exists is the data, not the evidence. The smaller live corpus remains the only one anything runs: thirty-five labelled cases over twenty-three entities (`resolution_corpus.py`, `resolution_cases.py`), which is what `RESOLUTION_CALIBRATION.md` measures. On benchmarks the row's finding stands and is narrowed: no benchmark *record* exists in any form. Corrected 2026-08-19: this row said `p50` and `p95` "appear nowhere in `tests/` or `docs/` for this plane"; they appear in `relationship-intelligence-v0.3.md` and its acceptance matrix, which are about this plane. What is true is the narrower claim — nothing measures them. Corrected 2026-08-19: this row also said a search for `benchmark` returns no hits, and that is now false — the token appears at `program_scale_cases.py:51` and `program_scale_corpus.py:742` — and elsewhere in the tree besides, so "twice" was itself an unscoped search result, which is the defect this sentence corrects, both in comments anticipating a benchmark that was never written. **This work package remains unmet**, and is the second half of `RI-PR135-BLOCKER-006` alongside the acceptance-matrix substitution. **And the acceptance matrix disagrees with this row.** [`relationship-intelligence-v0.3-acceptance.md`](../specs/relationship-intelligence-v0.3-acceptance.md) scores `RI-AC-031` as `MET` at tier `UNIT`. The *criterion* is defensibly met — the fixture clears every stated minimum, which loading it confirms — but the *tier* is not: no test of any tier imports either module, so there is no unit-tier evidence to cite. **Resolved 2026-08-19, and the earlier reason for not resolving it was false.** This cell previously said that file "is a specification document and is not edited from an implementation change". No such rule exists in `AGENTS.md` or `CONTRIBUTING.md`; this PR *created* that file and edited two other documents under `docs/specs/` in the same campaign. The tier is corrected there to none, since no test imports the fixture. Recorded rather than quietly amended because inventing a boundary and then citing it as the reason a defect stays open is the same class of defect as the ledger entries this section exists to correct: where the two differ on a *disposition* this row is the one with the evidence behind it — though on the composition figures it was the file that had them right, and this row that did not | A test that actually reads the program-scale fixture, and every p50/p95 benchmark this work package requires. The fixture half is built but unexercised; the benchmark half does not exist |
| **WP-RI-13** — documentation, independent review, rollout package, handoff: current `docs/specs/` RI spec, `docs/planning/` implementation/rollout record, MCP tool documentation, schema/data lifecycle, threat/privacy notes, backfill operator procedure, feature-flag/grant matrix, benchmark results, acceptance matrix, rollback/recovery procedure | Each of the ten named documents, current | Mixed. `docs/specs/` RI spec: v0.2 is the current requirements source and remains so; [`relationship-intelligence-v0.3.md`](../specs/relationship-intelligence-v0.3.md) is mirrored beside it as a proposed successor awaiting operator review, so the "current spec" item is met by v0.2 and the successor is filed but not yet in force. Corrected 2026-08-19: this cell said v0.3 "is now current" and called v0.2 "wrong lineage", which asserted an acceptance v0.3's own front matter says has not happened. Implementation/rollout record: this document, under `docs/plans/` rather than a `docs/planning/` directory — present, but see the reading rule above for what in it is legacy-numbered. MCP tool documentation: no standalone doc; tool semantics are described in prose in this document and in `ops/runbooks/relationship-intelligence.md`. Schema/data lifecycle: no dedicated doc; described piecemeal across this document and the runbook. Threat/privacy notes: `docs/security/threat-model.md` exists and names relationship identity (e.g. `ABUSE-PKL-019`), but is scoped to the original MCV read-only slice and has not been revisited for the entity plane specifically. Backfill operator procedure: does not exist, because there is no backfill framework to operate (see WP-RI-08). Feature-flag/grant matrix: reasonably covered informally by `ops/runbooks/relationship-intelligence.md` sections 1–2. Benchmark results: do not exist (see WP-RI-12). Acceptance matrix: **added by this remediation cycle** — [`relationship-intelligence-v0.3-acceptance.md`](../specs/relationship-intelligence-v0.3-acceptance.md) scores v0.3's `RI-AC-001..040`; section 3 below scores this campaign's own substituted forty and is the record of what the campaign believed it was proving. Neither supersedes the other while v0.3 is unaccepted: the two answer different documents. Rollback/recovery procedure: not present as a named section; the runbook covers disabling the feature flag but not recovering from a bad migration or a bad merge decision | A standalone MCP tool doc, a schema/data-lifecycle doc, a refreshed threat-model pass scoped to the entity plane, a backfill operator procedure (once WP-RI-08 exists), benchmark results (once WP-RI-12 exists), and a rollback/recovery procedure. Six of ten items outstanding in some form — the six named in this cell's remaining column; two corrected by this cycle |
...
invented, and that document answers a proposed successor that is not yet in
...
| Claim | Evidence |
|---|---|
| FAST tier green | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **7931 passed, 0 failed, 953 deselected** |
| Lint and format | `ruff check .` and `ruff format --check .` — clean over 919 files |
| Types | `mypy` (configured targets: `src`, `migrations`, `apps`, `ops`) — clean over 337 files |
| Full database tier green | `pytest -m "database or recovery or e2e"` against a live PostgreSQL 17 — **943 passed, 0 failed**. Measured in three disjoint chunks — `tests/database` (205), `tests/schema` (338), and the remainder (400) — because the single-command run was killed twice mid-flight at 84% and 53%, and a partial run is not evidence. The chunks partition the same selection and sum to 943. Stated this way rather than as one number, because it was not one command. The four schema-suite failures adversarial review surfaced (accumulation sets in the audit, enrollment, capture and entity migration tests, outgrown by this plane's eight tables and five capabilities) are fixed, not deselected |
...
**Three rounds so far, not one.** Each is four reviewers, one per lens, run by
...
**Twenty-two guards across three rounds were correct and untested**, not nine across
one. An earlier version of this section recorded only round 1 and reported the
nine — while the commit that added this section's own corrections was titled for
round 2. A section whose purpose is to record independent review is the worst
place to omit one.
