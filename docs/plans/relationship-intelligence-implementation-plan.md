...
| Full database tier disposition | **FULL_DB_TIER_NOT_RUN_TARGETED_EVIDENCE_SUFFICIENT.** The immutable-origin correction used a separately verified, localhost-only, auto-removed PostgreSQL 17.10 container for 82 focused tests; the proposal-token correction recreated the same bounded target pattern for five unequal-version cases. Neither authorized or ran the full database tier. The `database or recovery or e2e` selection collects 1,977 tests on this tree — corrected 2026-09-02 from 1,551, which was already stale by 412 before RI-ENT-WP-13 touched anything: that selection collected 1,963 at the RI-ENT-WP-08 head this branch is based on, and RI-ENT-WP-13's `tests/database/test_tbr_completeness_fixture.py` adds the remaining 14. Recomputed by `--collect-only` against this exact tree rather than by adding a delta to the stale figure. This figure is bound by no guard, which is why it was able to drift; it is corrected here for honesty, not because a test demanded it; collection is not execution evidence for the unselected tests. Targeted schema, migration, SQL-structure, concurrency, and database execution evidence is recorded in the campaign completion report. |
