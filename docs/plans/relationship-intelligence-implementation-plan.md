...
| Governance holds at the server | `tests/database/test_entity_governance.py` — 28 tests: a proposal cannot be accepted without an actor in either direction; a decided proposal cannot be decided again (asserted at the repository, below the service's own check); a merge record cannot cite another Principal's proposal |
...
* **`normalized_value` is disclosed and is not a redaction of `observed_value`.**
  `entities.unresolved_mentions` withholds the observed text and publishes the
  matched form. `normalize_name` casefolds and turns punctuation into spaces and
  removes **no content**, so a writer that passes `normalize_name(<raw lifted
  text>)` publishes that text with its dots turned into spaces — and the result
  is `is_normalized_name`-true, so no predicate over the stored string can tell
  it from a long legitimate name. The obligation is therefore on the writer and
  is stated on `EntityRepository.record_observation`; `_require_normalized_name`
  on the write and read paths is necessary and **not sufficient**, and says so.
  Nothing in `src/` writes this table, so the queue is empty on every build this
  branch can produce and the exposure is prospective — it lands with ingestion.
  An independent reviewer proposed the structural fix: a separate nullable
  column read only by the queue, so a writer that does nothing deliberate
  discloses nothing. It is an additive migration over zero rows today and is not
  free once ingestion lands. **Open, and deliberately an operator decision.**
