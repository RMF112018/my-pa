...
| Claim | Evidence |
|---|---|
| FAST tier collection and prior execution | Current marker-free combined-tree collection for `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` is 9,230 nodes; all **9,230 passed-or-not-currently-executed nodes are accounted for** by collection, partitioned exactly as `tests/architecture` (3,991) plus the remainder (5,239). Architecture tier **3,991 passed-or-not-currently-executed nodes** are likewise a collection statement, not a current runtime-pass claim. Before rebase, the RouteLLM exact head executed 9,071 nodes successfully (3,976 architecture plus 5,095 remainder), while the independently reviewed WP-FE-03 tree executed 9,113 of its 9,115 nodes successfully (3,926 architecture plus 5,187 remainder); its other two Trio-dependent nodes were separately unavailable because the optional `trio` dependency was not installed. Those executions are historical evidence for their respective exact trees and are not asserted for this combined tree. The collection equality remains enforced, while execution and environment availability are stated separately. |
