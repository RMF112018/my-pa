...
| Types | `mypy` (configured targets: `src`, `migrations`, `apps`, `ops`) — clean over 408 files. Re-derived on the Phase B plus GSQS merge candidate from the configured target tree. |
...
| Migrations apply and reverse | `tests/schema/test_entity_schema_migration.py` derives the current inventory. Phase B now owns seven tables: the prior five proposal/review/identity tables plus `relationship_write_requests` and its evidence child on `3d07af4dc513`. The cumulative revision also carries the B1/B4/B5 column, constraint, backfill and trigger deltas and is the single head. Database execution still requires the disposable PostgreSQL fixture; collection is not execution evidence. |
