...
| Claim | Evidence |
|---|---|
| FAST tier green | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **9798 passed, 0 failed, 1041 deselected**, measured in two chunks — `tests/architecture` (4,024) and the rest of the selection (5,774), which sum to 9,798 and partition it exactly — because the single-command run is killed in this environment for the same reason the database tier is chunked. Architecture tier **4024 passed**. Both figures were stale at the head that corrected the database-tier row below: that commit added three architecture tests and refreshed neither, which is the same defect it was written to fix, one row up. **It then happened again**, at the very next head: this cell said 8080 for a tier collecting 8082, stale by exactly the two tests the same commit added — because the guard beside it was an upper bound and admitted every understatement. It is an equality now, and a claim that must sit below collection states its own skip count. **Re-derived 2026-08-22 for WP-29, and the method is stated because it is weaker than the row above it:** the Relationship Memory plane added five test modules and this cell's figures were refreshed from `pytest --collect-only` on the selection this sentence names, not from a re-execution of the tier in this environment. Collection is what the guard beside this table binds, so the numbers are true of what the selection holds; the pass column is carried forward from the previous execution and is a claim about a tier that has not been re-run here |
