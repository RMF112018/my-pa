...
| Claim | Evidence |
|---|---|
| FAST tier green | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **10125 passed, 0 failed, 1108 deselected**, executed as one command on 2026-08-23 after adding the two MCP interoperability contract cases and the Synology `DEFAULT_FORWARD` redirection regression. The selection now consists of `tests/architecture` (4,127) and the rest (5,998), which sum to 10,125 and partition it exactly. Architecture tier **4127 passed**. Both figures were stale at the head that corrected the database-tier row below: that commit added three architecture tests and refreshed neither, which is the same defect it was written to fix, one row up. **It then happened again**, at the very next head: this cell said 8080 for a tier collecting 8082, stale by exactly the two tests the same commit added — because the guard beside it was an upper bound and admitted every understatement. It is an equality now, and a claim that must sit below collection states its own skip count. **Re-executed 2026-08-22 at the `origin/main` merge that brought in WP-FE-03**, not merely re-collected: both chunks were run at that commit and passed 4,072 and 5,961 of the 4,072 and 5,961 they collect; the architecture figure has since moved to 4,127, the twenty-two tests being the per-branch split `RM-API-AC-002` needed, the two further blind-spot sweeps beside it, and the firewall redirection regression. The previous figures — 9,844 in two chunks — were this tier before that merge added the Work Task and Commitment suites; they are superseded rather than corrected, because nothing about them was false when written. The deselected figure moved from 1,101 to 1,108 without either passing figure moving, which is what a database-tier addition looks like from here: the seven are the seven dispositions `tests/database/test_relationship_memory_review.py::test_every_disposition_writes_exactly_the_memory_tables_it_is_declared_to` is parametrized over, and the row below carries them. The distinction matters because the row below it was, for two heads, a collection figure written into a column headed "as executed" |
