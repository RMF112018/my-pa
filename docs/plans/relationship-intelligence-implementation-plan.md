...
and ten rounds have now run — dispatched to reviewers holding no part of this
...
**Ten rounds so far, not one.** Each is four reviewers, one per lens, run by
...
* **Carried, and NOT this branch's — nine commands read a caller-supplied
  ...
  `conversation_context` into a handler that assumes a string. All nine are
