...
| Claim | Evidence |
|---|---|
| FAST tier green | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **13,941 passed; 1,539 deselected**, as one command on the Phase B plus ChatLLM-client GSQS merge candidate. Architecture tier **4,611 passed**, run as its own command. **Both figures equal what their selections collect, and nothing is skipped**, which is what the guard beside them requires and what makes them checkable rather than reported. The previous Phase B plus GSQS plus PM-157-H-001 entry — **13,878 passed; 1,539 deselected**, architecture 4,610 — and the ChatLLM-client-driven GSQS B0 entry — **12,564 passed**, architecture 4,439 — were true of their separate trees and are superseded by this combined-tree measurement. |
...
| Types | `mypy` (configured targets: `src`, `migrations`, `apps`, `ops`) — clean over 418 files. Re-derived on the Phase B plus ChatLLM-client GSQS merge candidate from the configured target tree. |
| Full database tier green | `pytest -m "database or recovery or e2e"` against a disposable PostgreSQL 17.10 — **1,528 passed**, as one complete selection on the Phase B plus ChatLLM-client GSQS merge candidate. The figure equals collection, so nothing is skipped. Earlier Phase B and GSQS database figures remain valid only for their separate trees and are superseded here. |
...
| MCP surface | `tests/contract/test_mcp_transport.py` — a real child process composed for the plane and its write half publishes all 102 tools, and an unconfigured one publishes 56, withholding the thirty-one `entities.` names along with the six `documents.` names and the nine `relationship_memory.` names. Corrected 2026-08-23: this cell said 54, which was the whole capability set before the Intelligence Artifact and Relationship Memory planes and is now neither of the two figures this test asserts. Neither number is written into the test — both are derived from `Capability` there, which is why the test stayed green while the cell describing it went stale |
