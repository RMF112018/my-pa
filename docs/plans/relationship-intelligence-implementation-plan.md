...
| Claim | Evidence |
|---|---|
| FAST tier validation | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **14,239 passed** on the final currency-alignment corrective tree; exact duration and deselection count are recorded in the campaign completion report after the frozen execution. |
