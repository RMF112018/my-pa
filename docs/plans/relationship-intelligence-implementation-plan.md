...
| **WP-RI-06** — observation, proposal, review, and merge workflows; review integrated into the *existing* review plane; merge preview; transactional redirect; audit and receipt; replay protection; expected-version checks; scheduled agents excluded from merge apply | A governance service reachable by some caller, whose review path is the repository's one review mechanism | **Completed by Phase B.** Entity and Relationship Memory candidates appear on canonical `review.list` and all governed dispositions run through `review.decide`; ordinary accepted proposals promote through canonical services, while accepting an identity-correction proposal does not merge. `entities.merge.preview` persists the exact-version-bound, expiring consequence plan and `entities.merge` applies that exact preview atomically under operator authority, audit, replay, conflict, and effect-ledger controls. Local and remote entry points are composed; remote merge remains off by default and requires global remote writes plus an exact server-resolved `remote.operator` durable capability set and all ordinary capability/purpose/feature/policy gates | `WP-07` split execution, `WP-08` final Relationship Memory redistribution, `WP-10` complete cross-plane re-enrichment, frontend, live grants/flags, deployment, and live data remain outside this completed work package |
...
| Claim | Evidence |
|---|---|
| FAST tier green | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **13,947 passed; 1,539 deselected**, as one command on the ChatLLM compact MCP gateway Gate-2 nested-opacity *args/**kwargs/bound-method/match and CI-identity corrective plus Phase B plus GSQS plus PM-157-H-001 tree. Architecture tier **4,642 passed**, run as its own command. **Both figures equal what their selections collect, and nothing is skipped**, which is what the guard beside them requires and what makes them checkable rather than reported. The previous compact-gateway nested-opacity entry — **13,939 passed, 1,539 deselected**, architecture 4,636 — the previous compact-gateway entry — **13,931 passed, 1,539 deselected**, architecture 4,628 — the previous compact-gateway first-corrective entry — **13,927 passed, 1,539 deselected**, architecture 4,627 — the previous combined-tree entry — **13,878 passed, 1,539 deselected**, architecture 4,610 — the previous Phase B entry — **12,813 passed, 1,467 deselected**, architecture 4,326 — the concurrent GSQS entry — **12,034 passed**, architecture 4,323 — and the later main-only PM-157-H-001 measurement — **12,504 passed**, architecture 4,438 on `a92bedd` — were true of their separate trees and are superseded by this measurement. Earlier red and superseded campaign figures remain preserved in Git history rather than presented as current evidence. |
...
| Types | `mypy` (configured targets: `src`, `migrations`, `apps`, `ops`) — clean over 418 files. Re-derived after adding the compact ChatLLM façade module. |
| Full database tier green | `pytest -m "database or recovery or e2e"` against a disposable PostgreSQL 17.10 — **1,528 passed**, as one complete selection on the Phase B plus GSQS merge candidate. The figure equals collection, so nothing is skipped. Earlier Phase B and GSQS database figures remain valid only for their separate trees and are superseded here. |
...
| MCP surface | `tests/contract/test_mcp_transport.py` — a real child process composed for the plane and its write half publishes all 101 tools, and an unconfigured one publishes 55, withholding the thirty-one `entities.` names along with the six `documents.` names and the nine `relationship_memory.` names. Corrected 2026-08-23: this cell said 54, which was the whole capability set before the Intelligence Artifact and Relationship Memory planes and is now neither of the two figures this test asserts. Neither number is written into the test — both are derived from `Capability` there, which is why the test stayed green while the cell describing it went stale |
| Remote exposure | `tests/contract/test_entity_remote_exposure.py` and `tests/contract/test_remote_mcp_transport.py` — no `entities.` name reaches the remote profile with the plane off. With it on, the ten reads remain reads and all twenty-one writes require `remote_writes_enabled`. The ordinary profile and a raw capability allowlist still exclude `entities.merge.preview` and `entities.merge`; they join only when the server-resolved durable capability set exactly equals frozen `remote.operator`, and invocation still intersects allowed capabilities and exact purpose grants before application feature, Principal, policy, and audit checks. Writes disabled, an absent/mismatched/subset/superset/non-operator profile, an empty grant, or a missing purpose each refuses causatively. No live profile, grant, or flag is activated |
...
| 7 | MED | The runbook instructed operators to run `EntityGovernanceService` and `EntityReenrichmentService`, which **nothing in `src/` composed at that head** | Corrected at the time by stating the gap; **superseded by Phase B**, which composes canonical `review.list`/`review.decide` and operator-only merge preview/apply entry points. `WP-10` complete re-enrichment remains deferred rather than implied by those entry points |
...
* **Proposal review and governed merge now have callers.** Entity and
  ...
  `entities.merge.preview` and `entities.merge` are the separate operator-only
