...
| Claim | Evidence |
|---|---|
| FAST tier green | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **10518 passed, 0 failed, 1108 deselected**, measured in two chunks — `tests/architecture` (4,234) and the rest of the selection (6,284), which sum to 10,518 and partition it exactly — because the single-command run is killed in this environment for the same reason the database tier is chunked. Architecture tier **4234 passed**. Collection was refreshed for GSQS ChatLLM remote-eval Phase A (FAST 10,124→10,363, +239, not +233; architecture 4,126→4,203), which added the remote-eval unit/architecture modules — those modules collect 169 tests; 233 double-counted the 64 parametrized legal/illegal transition cases the collector already expands. The 10,124 base already includes the two MCP interoperability contract cases from #151; focused Phase A tests were executed on that tree. Both figures were stale at the head that corrected the database-tier row below: that commit added three architecture tests and refreshed neither, which is the same defect it was written to fix, one row up. **It then happened again**, at the very next head: this cell said 8080 for a tier collecting 8082, stale by exactly the two tests the same commit added — because the guard beside it was an upper bound and admitted every understatement. It is an equality now, and a claim that must sit below collection states its own skip count. **Re-executed 2026-08-22 at the `origin/main` merge that brought in WP-FE-03**, not merely re-collected: both chunks were run at that commit and passed 4,072 and 5,961 of the 4,072 and 5,961 they collect; the architecture figure has since moved to 4,126, the twenty-two tests being the per-branch split `RM-API-AC-002` needed and the two further blind-spot sweeps beside it. The previous figures — 9,844 in two chunks — were this tier before that merge added the Work Task and Commitment suites; they are superseded rather than corrected, because nothing about them was false when written. The deselected figure moved from 1,101 to 1,108 without either passing figure moving, which is what a database-tier addition looks like from here: the seven are the seven dispositions `tests/database/test_relationship_memory_review.py::test_every_disposition_writes_exactly_the_memory_tables_it_is_declared_to` is parametrized over, and the row below carries them. The distinction matters because the row below it was, for two heads, a collection figure written into a column headed "as executed" |
...
| Types | `mypy` (configured targets: `src`, `migrations`, `apps`, `ops`) — clean over 383 files |
| Full database tier green | `pytest -m "database or recovery or e2e"` against a live PostgreSQL 17 — **1098 passed, 0 failed**, in three disjoint chunks: `tests/schema` (338), `tests/database` (334) and the remainder (426). The marker selection collects **1098**, and 338 + 334 + 426 is 1098, so the chunks partition it exactly. **Re-executed 2026-08-23 on this branch against a disposable PostgreSQL 17 — all three chunks, not only the one that moved**, because a tier figure assembled from one fresh chunk and two remembered ones is a collection figure wearing an execution's words, which is the defect this cell already records once. The figure rose from 1,091 by exactly seven: `tests/database/test_relationship_memory_review.py` gained `test_every_disposition_writes_exactly_the_memory_tables_it_is_declared_to`, one test parametrized over `Disposition` itself — seven cases, the population read off the enum — which reads the memory-plane tables each disposition writes off the SQL the server is actually sent rather than off a walk of the source. **Executed 2026-08-22 at the `origin/main` merge that brought in WP-FE-03 against a disposable PostgreSQL 17, and the three chunks are what was run rather than what was planned.** Earlier revisions of this cell were collection figures in a column headed "as executed"; this one is not. The chunking is restated as the three the selection now partitions into rather than the four it did, because a chunk list is a description of a run and the previous one no longer describes this selection. The arrangement changed again when the report plane merged: `tests/database` as one chunk had run before and was killed twice here, in the background and with no output at all, on a machine reporting 42% free memory — so it was split and both halves run in the foreground. Four rather than five is not a smaller run; it is a different one, and the figures are this arrangement's. The chunk list is written as what was run rather than as what was planned, because a partial run is not evidence and a chunking that was not used is not a measurement. Chunked because the single-command run is killed in this environment — twice at 84% and 53%, once more at 63% — and a partial run is not evidence. **Corrected 2026-08-20, and the correction is the point.** This cell previously claimed **1,926 passed** across four chunks. That number was real in the sense that those commands ran and those tests passed, and false as written: the chunks were whole directories run with **no `-m` filter**, so they were a different and larger selection than the one the cell named, and no partition of a 951-test selection can sum to 1,926. An independent reviewer caught it by running `--collect-only` on the selection the sentence named — which is all it took, and which nothing in the repository does for a bare number. The four schema-suite failures adversarial review surfaced (accumulation sets in the audit, enrollment, capture and entity migration tests) are fixed, not deselected. A fifth, of the same kind, was surfaced by running the tier: `entities.unresolved_mentions` had its forward `ALTER` and head admitted it, but `CAPABILITIES_ADDED_AFTER_THE_CAPTURE_REVISION` did not list it, so the one test that can catch a capability added *without* an `ALTER` reddened on a capability that had one |
...
| The plane refuses when it is off | `tests/contract/test_entity_capabilities.py` — every one of the six, parameterized, answers `unsupported` on a build that never enabled the relationship plane. Parameterized deliberately: the floor was missing from all five, so a test covering one would have gone green over four open holes. `tests/contract/test_http_transport.py` composes the plane explicitly, having previously asserted `200` for capabilities the same process reported `not_implemented` |
| MCP surface | `tests/contract/test_mcp_transport.py` — a real child process publishes all 54 tools when composed for the plane and withholds the six when not |
| Remote exposure | `tests/contract/test_entity_remote_exposure.py` — none of the six reaches the remote profile with the plane off, under either write setting |
