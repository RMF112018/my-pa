...
| Claim | Evidence |
|---|---|
| FAST tier validation | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **14,242 passed** on the final immutable-origin corrective tree. |
| Architecture tier | Architecture tier **4,712 passed** on the same integrated corrective tree. This path-selected figure is stated separately so its checked claim cannot be mistaken for the FAST marker result. |
...
| Full database tier disposition | **FULL_DB_TIER_NOT_RUN_TARGETED_EVIDENCE_SUFFICIENT.** The immutable-origin correction used a separately verified, localhost-only, auto-removed PostgreSQL 17.10 container for 82 focused tests; it did not authorize or run the full database tier. The `database or recovery or e2e` selection collects 1,548 tests on this tree; collection is not execution evidence for the unselected tests. Targeted schema, migration, SQL-structure, concurrency, and database execution evidence is recorded in the campaign completion report. |
