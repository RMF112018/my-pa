...
| Claim | Evidence |
|---|---|
| FAST tier green | `pytest -m "not slow and not database and not network and not connector and not evaluation and not e2e and not recovery"` — **9841 passed, 0 failed, 1069 deselected**, measured in two chunks — `tests/architecture` (4,056) and the rest of the selection (5,785), which sum to 9,841 and partition it exactly — because the single-command run is killed in this environment for the same reason the database tier is chunked. Architecture tier **4056 passed**. Both figures were stale at the head that corrected the database-tier row below: that commit added three architecture tests and refreshed neither, which is the same defect it was written to fix, one row up. **It then happened again**, at the very next head: this cell said 8080 for a tier collecting 8082, stale by exactly the two tests the same commit added — because the guard beside it was an upper bound and admitted every understatement. It is an equality now, and a claim that must sit below collection states its own skip count. **Re-executed 2026-08-22 for WP-29**, not merely re-collected: the tier was run against `f4d05f5` and passed 9,799 of 9,799, and the figures here are that run refreshed by collection after `origin/main` merged in, which added one contract module. The distinction matters because the row below it was, for two heads, a collection figure written into a column headed "as executed" |
