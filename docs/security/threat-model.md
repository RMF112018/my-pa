...
**Transport.** Local MCP remains deliberate `local_operator` stdio: it opens no socket, accepts no caller-supplied Principal, and its kill switch empties `tools/list` and refuses `tools/call`. A separately composed persistent remote MCP surface now exists over Streamable HTTP but is disabled by default. It authenticates through the origin OAuth server with protected-resource metadata and resource/scope-bound tokens, applies exact host/origin and DNS-rebinding protections, requires the durable database remote-security control and capability/purpose grants, and keeps remote writes behind an independent global switch. Operator-only merge additionally requires the exact server-resolved `remote.operator` durable capability set; raw allowlists and non-operator profiles do not confer it. This implementation creates no live client, grant, public ingress, flag activation, or deployment; those external actions remain operator-gated.
