...
- Vitest — **39 files / 433 tests passed** under the exact-lock runtime.
...
- Full exact-source Playwright selection, including `work-mutations.spec.ts` — **170 passed / 2 expected project-specific target-size skips / 2 dynamic visual failures**, 3.0 minutes, across desktop/tablet/mobile. The new mutation/security journeys passed **9/9** and cover Task and Commitment create/read/close, safe edit, concurrent conflict/reapply, link/unlink/relink, terminal evidence and receipt linkage, explicit reveal, cross-object independence, caller-Principal refusal, nondisclosing foreign IDs, and explicit unsupported/unavailable state.
- The two full-run failures were independently confined to the layout width retained by a hidden dynamic freshness timestamp; no product pixels outside the shifted `Truncation` label differed. After normalizing only that hidden test value without changing production UI or snapshot bytes, a fresh exact-head desktop/tablet/mobile no-update run passed **3/3**. Together these runs account for every configured browser node without treating the transient visual delta as a product pass.
...
- Full database/recovery/e2e marker tier — **991 passed / 9,272 deselected / 0 failed** in 44:24 against a fresh disposable PostgreSQL database. Exact migration denotation and stale-head inventories were corrected; the database was dropped and independently verified absent. The measured duration justified raising the CI job budget from 40 to 55 minutes without reducing coverage.
- Visual snapshots — the independent visual review passed the exact-source desktop/tablet/mobile 200%-reflow states. The dynamic freshness value remains truthful semantic `<time>` content at runtime; only its hidden screenshot value is normalized to a fixed inline string so it cannot shift adjacent static text. Existing reviewed baseline bytes passed **3/3** without update.
