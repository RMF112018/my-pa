...
| RM-API-AC-002 each capability has a grant boundary appropriate to the rows it reaches | PASS | The eight `relationship_memory.*` capabilities carry two purposes of their own — `relationship_memory_authoring` over the four writes, `relationship_memory_read` over the four reads — and neither is a reuse: admitting a memory read under `entity_read` would let a grant issued to learn who someone *is* also return what the Principal privately wrote about them, and admitting a memory write under `capture_authoring` would let a grant issued to store a Quick Note write an entity-bound assertion about another person. `POL::test_every_capability_refuses_a_purpose_it_does_not_permit` puts every capability through every purpose it does not hold and asserts the refusal. **A ninth capability reaches memory rows and is not one of the eight: `entities.context`, whose only purpose is `entity_read`.** The context card carries each memory's `statement` verbatim alongside its kind, authority, classification, `pinned` flag, applicability window and `recorded_at` — so an `entity_read` grant *does* return memory text, which is the disclosure the purpose split above exists to prevent everywhere else, and this row said the opposite by saying nothing. What bounds it is the classification boundary rather than the purpose name, and that boundary is narrower than a memory read in five ways: the card answers for one subject the caller already named and can already read; it carries at most `CONTEXT_CARD_COLLECTION_LIMIT` (25) memories; `summaries_for_context` composes `_mine` over both memory tables and filters `lifecycle_state = active`, so it reaches only the acting Principal's own live memories; every `restricted_local` version is dropped and merely *counted*, and the count leaves the application as the boolean limitation `memories_were_withheld_by_classification` — the boolean is what the application emits, and it is **not** a bound on the count. `summaries_for_context` in `src/my_pa/infrastructure/persistence/relationship_memory.py` applies the LIMIT *before* the classification filter: it selects `limit + 1` rows, keeps the first `limit`, and only then drops the restricted ones, so on a truncated card the caller recovers the withheld count arithmetically as `CONTEXT_CARD_COLLECTION_LIMIT - len(card["memories"])`. Both inputs are on the wire — the memory list itself, and the `more_memories_than_this_card_carries` limitation that says the page was full — and the `pinned.desc()` ordering guarantees a pinned restricted row consumes a window slot rather than falling past it. `CARD::test_a_page_both_truncated_and_thinned_states_both` asserts that arithmetic on this branch already, `len(card["memories"]) == CONTEXT_CARD_COLLECTION_LIMIT - 2` alongside both limitations, so a reader can check the claim rather than take it. It is accepted and disclosed rather than closed, on the repository's own stated reason: the comment above the `withheld` computation holds that a card is one entity the caller already named and already reads, so "three memories are withheld here" discloses no existence the caller did not have, while the same count in a search would let a term probe find one. The boolean narrows that decision and does not bound it, and the line is exactly this — an `entity_read` grant can learn how many of the acting Principal's **own** restricted memories sit in the top of one **named** subject's window; it cannot learn that a restricted memory exists on a subject it has not named, cannot read one, and cannot reach another Principal's at all; and the exclusion is a constructor invariant, not a `WHERE` clause — `ContextCardMemory.__post_init__` raises on a restricted version, so a second reader or a rewritten query cannot lose it. `entity_read` still reaches no revision history, no cross-subject search and no write. That this is design and not oversight is fixed by two things outside this row: `RM-AC-029` and `RM-API-AC-013` *require* the card to carry the memory collection and to advertise what it truncated or withheld, so removing the disclosure would fail them; and under ADR-003 the committed statement is itself the record — unlike an observation, whose value is evidence lifted out of someone else's mail and is left to `entities.get` — so a summary with the sentence removed would be a list of the fact that the Principal had written something. **A tenth capability reaches memory rows and writes them: `review.decide`, whose only purpose is `review_disposition`.** It reads no memory row, but `_promote` in `src/my_pa/infrastructure/persistence/relationship_memory_review.py` issues `insert(relationship_memories)` and `insert(relationship_memory_versions)` under `_bound`, so the enumeration of capabilities outside the eight is complete at two: one reads under `entity_read` and one writes under `review_disposition`. What bounds the write is what it cannot author: `_promotion_authority` produces only `user_confirmed_assertion` or `source_backed_assertion`, never `public_assertion` or `user_authored_private_note`, and `model_inference` and `unresolved_claim` are not `MemoryAuthority` members at all — together with the composition gate, since the memory branch of the decide router is not reached unless the plane is composed; `RM-API-AC-011` carries the promotion path itself and this row does not restate it. Bound by `CARD::test_a_carried_memory_renders_its_summary_fields` (the statement and every summary field are disclosed), `CARD::test_a_wholly_withheld_page_is_a_withholding_and_never_an_absence` (restricted rows are withheld and the card says so rather than reading as empty), `CARD::test_a_memory_summary_never_reaches_another_principals_card` (the partition), and `POL` |
