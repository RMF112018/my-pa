...
heads_output=$(database_operator python -m alembic heads | awk '{print $1}')
...
database_operator python -m alembic upgrade head
current=$(database_operator python -m alembic current | awk '{print $1}')
[ "$current" = "$heads" ] || { echo "post-migration revision mismatch" >&2; exit 1; }
database_operator python apps/cli/health.py
