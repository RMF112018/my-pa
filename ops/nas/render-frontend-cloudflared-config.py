#!/usr/bin/env python3
"""Render non-secret, exact-hostname frontend cloudflared configuration."""
...
def render(template: Path, output: Path, *, tunnel_id: str, hostname: str) -> None:
    ...
    if source.count("__TUNNEL_ID__") != 1 or source.count("__FRONTEND_HOSTNAME__") != 1:
        raise ValueError("unexpected template placeholders")
    rendered = source.replace("__TUNNEL_ID__", tunnel_id).replace("__FRONTEND_HOSTNAME__", hostname)
...
def main() -> int:
    ...
    parser.add_argument(
        "--template",
        type=Path,
        default=Path(__file__).with_name("frontend-cloudflared-config.example.yml"),
    )
