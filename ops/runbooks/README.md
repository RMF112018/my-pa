...
| [`mcp-project-management.md`](mcp-project-management.md) | Continuity Project MCP client contract: nested payload, `expected_version`, lifecycle, replay, participation via `project_id`, Task association. **UNEXECUTED** contract text. |
| [`project-entity-bridge-and-backfill.md`](project-entity-bridge-and-backfill.md) | Project↔Entity bridge, no name matching, unresolved_missing/ambiguous, forward mint on create. Production apply is operator-only and not authorized by this campaign. |
| [`mcp-project-runtime-validation.md`](mcp-project-runtime-validation.md) | Post-deploy read-only catalog/describe checklist for Project MCP. **UNEXECUTED / OPERATOR-GATED**. Does not authorize deploy. |
...
| [`production-frontend-deployment.md`](production-frontend-deployment.md) | Public browser origin `pa.bobby-fetting.me`: prepare/safe sequence (transport-only private smoke), operator-gated activate placeholders, rollback dry-run. **PRODUCTION_ACTIVATION_NOT_PERFORMED.** Private smoke does not claim WebAuthn/passkey/sign-in. |
| [`auth-runtime-validation.md`](auth-runtime-validation.md) | Operator-gated physical/runtime WebAuthn commissioning at `https://pa.bobby-fetting.me`. **UNEXECUTED / OPERATOR-GATED** (`NOT_PERFORMED_OPERATOR_GATED`). |
...
| [`relationship-intelligence.md`](relationship-intelligence.md) | The entity plane: what its fifty-five `entities.` names answer, including Principal-scoped identity history and governed merge/split preview/apply; how the plane, write, identity-correction, remote-write, exact `remote.operator`, durable capability/purpose, and policy gates compose around its seventeen reads and thirty-eight writes; and the unexecuted, operator-gated WP-08 commissioning and WP-09 canary procedures. Live activation remains deferred. |
