...
loopback. One hundred and seventy-one have application handlers. A default process serves
one hundred one: seventy handler-implemented capabilities are composition-withheld, and one
is structurally unwired, so 71 of 172 total capabilities are unavailable or unwired.
...
`71 of 172 total capabilities are unavailable or unwired.` rather than `ready`.
