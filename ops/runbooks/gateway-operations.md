...
`apps/gateway.py` routes the fifty-three public capabilities over HTTP on loopback
and, in a default process, serves forty-two of them.
...
**Corrected 2026-08-19: this line read "serves the fifty-three public
...
`MY_PA_MANAGED_DOCUMENT_ROOT` and the five `entities.` names behind
`MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED`, neither of which has a default.
`/v1/{capability}` is a path parameter, so all fifty-three *route*: dispatch
...
`11 of 53 capabilities are unwired.` rather than `ready`.
...
**Current-state correction (2026-08-15):** the candidate has **fifty-three**
capabilities and **sixty-two** Alembic revisions at head `d2b8f5c04e71`.
