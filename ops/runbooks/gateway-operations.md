...
`apps/gateway.py` routes the one hundred and four public capabilities over HTTP on
...
`MY_PA_MANAGED_DOCUMENT_ROOT`, the thirty-four `entities.` names behind
`MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED` (whose twenty-three writes need
...
capabilities and **eighty-three** Alembic revisions at head `8dc3619891bb`; the merged chain admits GSQS at `c4b0a1d9e827` before continuing Phase B at `c7a1f04b9e63`, and the corrective `b727e870d45e` is additive on `8e1c4a7b2d90`, with `7e114f822af2` additive on `b727e870d45e` and adding the `entity_names`/`entity_organization_profiles` tables (RI-ENT-WP-02), `441b071bf37b` additive on `7e114f822af2` and adding the `entity_addresses`/`entity_communication_methods` tables (RI-ENT-WP-03), `f5b06925857e` additive on `441b071bf37b` and adding the `entity_project_participations`/`entity_role_types`/`entity_discipline_types` tables (RI-ENT-WP-04), `17149a48fa30` additive on `f5b06925857e` and adding the `entity_person_organization_affiliations` table (RI-ENT-WP-05), and `8dc3619891bb` additive on `17149a48fa30` and adding the `entity_relationship_types` table, re-pointing `entity_relationships.relationship_type` at it by foreign key (RI-ENT-WP-06a). Revision figure corrected 2026-08-31.
