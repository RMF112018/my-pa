...
`apps/gateway.py` routes the one hundred and twelve public capabilities over HTTP on
...
`MY_PA_MANAGED_DOCUMENT_ROOT`, the forty-two `entities.` names behind
`MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED` (whose twenty-six writes need
...
`57 of 112 capabilities are unwired.` rather than `ready`.
...
**Current-state correction (2026-08-28):** the candidate has **one hundred and twelve**
