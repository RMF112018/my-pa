...
`apps/gateway.py` routes the one hundred and fifteen public capabilities over HTTP on
...
`MY_PA_MANAGED_DOCUMENT_ROOT`, the forty-five `entities.` names behind
`MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED` (whose twenty-nine writes need
...
`60 of 115 capabilities are unwired.` rather than `ready`.
...
**Current-state correction (2026-08-28):** the candidate has **one hundred and fifteen**
