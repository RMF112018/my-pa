...
`tests/contract/test_transport_parity.py` holds it over all one hundred and eighteen capabilities.
...
same as the tools this build implements. The build implements one hundred and eighteen, one
per capability name. A default process publishes **fifty-five**. That is the
118, less the six `documents.` names, the forty-eight `entities.` names
...
same child publishes those six, reaching sixty-one of the one hundred and eighteen. The
remaining fifty-seven are the forty-eight `entities.` names, which `D-RI-20`
gates behind `MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED` on exactly the same terms
— and whose thirty-two writes need
...
`MY_PA_RELATIONSHIP_MEMORY_ENABLED`; a child publishing all one hundred and eighteen has
...
**one hundred and eighteen** current capabilities, and the schema has
