...
`tests/contract/test_transport_parity.py` holds it over all forty-seven capabilities.
...
same as the tools this build implements. The build implements forty-seven, one per
...
same child publishes all forty-seven. An operator who expects `documents.create`
...
**forty-seven** current capabilities, and the schema has **fifty-three** revisions
at head `e8c1b5a7d204`. `capabilities.get` also reports content-free
