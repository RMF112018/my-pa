...
`tests/contract/test_transport_parity.py` holds it over all one hundred and twenty-one capabilities.
...
same as the tools this build implements. The build implements one hundred and twenty-one, one
per capability name. A default process publishes **fifty-five**. That is the
121, less the six `documents.` names, the fifty-one `entities.` names
...
same child publishes those six, reaching sixty-one of the one hundred and twenty-one. The
remaining sixty are the fifty-one `entities.` names, which `D-RI-20`
gates behind `MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED` on exactly the same terms
— and whose thirty-five writes need
...
`MY_PA_RELATIONSHIP_MEMORY_ENABLED`; a child publishing all one hundred and twenty-one has
...
**one hundred and twenty-one** current capabilities, and the schema has
