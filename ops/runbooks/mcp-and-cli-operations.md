...
`tests/contract/test_transport_parity.py` holds it over all one hundred and fifteen capabilities.
...
same as the tools this build implements. The build implements one hundred and fifteen, one
per capability name. A default process publishes **fifty-five**. That is the
115, less the six `documents.` names, the forty-five `entities.` names
...
same child publishes those six, reaching sixty-one of the one hundred and fifteen. The
remaining fifty-four are the forty-five `entities.` names, which `D-RI-20`
gates behind `MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED` on exactly the same terms
— and whose twenty-nine writes need
...
`MY_PA_RELATIONSHIP_MEMORY_ENABLED`; a child publishing all one hundred and fifteen has
...
**one hundred and fifteen** current capabilities, and the schema has
