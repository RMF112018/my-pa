...
| Read capabilities | Seventeen. The prior sixteen plus `UI-IMP-WP15`'s `entities.graph`, a seeded 1-hop or 2-hop neighborhood over assignments and relationships. `RI-ENT-WP-10` added the five family reads `entities.profile`, `entities.names.list`, `entities.addresses.list`, `entities.communication.list`, and `entities.participations.list` |
...
`MY_PA_RELATIONSHIP_INTELLIGENCE_ENABLED=true` makes the entity plane's seventeen
...
to `false`, so a process that enables the plane serves the seventeen reads and refuses
