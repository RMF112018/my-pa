...
| Purposes | Five. `entity_read` covers all sixteen reads; `entity_observation_ingest` covers `entities.observe`; `entity_authoring` covers the twenty-three writes it admits; `entity_proposal` covers the producer path; and `entity_identity_correction` covers merge and split preview/apply. Preview and apply intentionally share the correction purpose without collapsing their two-gate protocol |
...
reads available to the process, and nothing else: the twenty-nine writes need
