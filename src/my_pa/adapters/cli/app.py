...
fields, the same for all public capabilities — so it is presented as options,
which is what a CLI is for. The payload is capability-specific, so hand-written option sets
would be extra statements of what `application.commands`
already says, and a later capability would arrive with none. `--payload` is
