...
fields, the same for all thirteen capabilities — so it is presented as options,
which is what a CLI is for. The payload is capability-specific, so thirteen sets of
hand-written options would be thirteen statements of what `application.commands`
already says, and the fourteenth capability would arrive with none. `--payload` is
