...
MCP tools `goodnotes.work` and `goodnotes.propose`. It analyzes immutable
page-version content/context, returns schema-valid proposals, and has no direct
database, source, or destination writes. Canonical NEW-only summary delivery is
GN-06, not this Task. Fail closed when MCP, auth, or content transfer fails: do
