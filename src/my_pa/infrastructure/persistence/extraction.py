...
def extracted_text_in_scope(enrollment_id: str) -> tuple[ColumnElement[bool], ...]:
    ...
    **One list, and every statement that reads stored extracted text is built
    ...
    two, and `read_extraction` had been the third since before this file was
    last reviewed. A reader checking the claim would have found a caller the
    docstring did not know about, with no way to tell whether that was an
    omission or a violation. When evaluated against one statement snapshot,
    ...
    stood here for six review rounds and was false for two of the six
    conditions. A page of text beside a coverage report
    that says the scope
