...
def _environment(
    ...
    *,
    ...
    iptables.write_text(
        ...
        f"    value=$(cat '{state}')\n"
        '    case "$value" in\n'
        f"      effective) printf '%s\\n' '-A FORWARD_FIREWALL -s {SUBNET} -d {SUBNET} "
        f"-i {BRIDGE} -o {BRIDGE} -j RETURN' '-A FORWARD_FIREWALL -j DROP' ;;\n"
        f"      misordered) printf '%s\\n' '-A FORWARD_FIREWALL -j DROP' "
        f"'-A FORWARD_FIREWALL -s {SUBNET} -d {SUBNET} -i {BRIDGE} -o {BRIDGE} -j RETURN' ;;\n"
        f"      duplicate) printf '%s\\n' '-A FORWARD_FIREWALL -s {SUBNET} -d {SUBNET} "
        f"-i {BRIDGE} -o {BRIDGE} -j RETURN' '-A FORWARD_FIREWALL -s {SUBNET} "
        f"-d {SUBNET} -i {BRIDGE} -o {BRIDGE} -j RETURN' ;;\n"
        "      missing) printf '%s\\n' '-A FORWARD_FIREWALL -j DROP' ;;\n"
        "    esac ;;\n"
