...
def test_current_state_docs_name_the_current_capability_and_migration_counts() -> None:
    ...
    for label, path in documents.items():
        ...
        assert "forty-seven capabilit" in text.lower().replace(" public ", " "), (
            f"{label} lost the current capability count"
        )
        assert "fifty" in text, f"{label} lost the current revision count"
        assert "c3e9a7f1b204" in text, f"{label} lost the current Alembic head"
...
def test_web_readme_names_the_routes_and_capabilities_the_bff_reaches() -> None:
    ...
    assert "forty-seven capability names" in lowered
