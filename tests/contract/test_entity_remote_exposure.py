...
The entity plane is the case where that matters most: these five read who a
...
* a process that has not enabled the plane exposes none of the five, remotely or
  locally, however the write switch is set;
* a process that has enabled it exposes all five as **reads**, so they are
...
def test_the_five_names_are_the_family_this_file_is_about() -> None:
    """Guards the rest: an empty family would make every assertion below vacuous."""
    assert len(ENTITY_CAPABILITIES) == 5
...
def test_a_build_without_the_plane_still_serves_everything_else() -> None:
    """The gate withholds five names, not the process.
...
def test_a_build_with_the_plane_exposes_all_five_as_reads() -> None:
    """Reachable with writes disabled, because none of the five writes."""
    remote = remote_tool_names(_service(enabled=True), writes_enabled=False)
    assert remote >= ENTITY_CAPABILITIES


def test_none_of_the_five_is_classified_as_a_write() -> None:
