...
`MY_PA_RELATIONSHIP_INTELLIGENCE_WRITES_ENABLED` decides whether its twenty-nine
...
* plane off — all forty-five `entities.` names refuse, reads included;
* plane on, writes off — the sixteen reads answer and the twenty-nine writes refuse;
* plane on, writes on — all forty-five are served.
...
and `twenty-nine` are `_ENTITY_CAPABILITIES` split by the purpose map, and the
...
a thirtieth write mapped to `entity_authoring` joins this sweep on arrival. It
...
def test_the_write_set_the_service_subtracts_is_the_set_with_a_write_purpose() -> None:
    ...
    # Twenty-nine after `RI-ENT-WP-11`'s first two record families: the
    # twenty-three that final identity recovery left plus three verbs per family.
    assert len(ENTITY_WRITES) == 29
