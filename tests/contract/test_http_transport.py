...
**Reachability.** Every one of the one hundred and two capabilities is addressable
over HTTP and answers. Parametrised over `Capability` rather than over a list
written here, so a one-hundred-third capability added to the domain arrives as
a failing row instead of as an untested one. Eleven of the one hundred and two answer a
