...
**Reachability.** Every one of the one hundred and seventy-two capabilities is addressable
...
def payloads_for(scene: Scene, record: KnowledgeRecord) -> dict[Capability, dict[str, Any]]:
    ...
    return {
        ...
        Capability.TASKS_COMMENTS_CREATE: {
            "task_id": task.task_id,
            "body": "HTTP task-plane comment",
            "idempotency_key": "http-task-comment-create-0001",
        },
...
def commands_for(
    ...
    return {
        ...
        Capability.TASKS_COMMENTS_LIST: ListTaskComments(task_id=task.task_id),
