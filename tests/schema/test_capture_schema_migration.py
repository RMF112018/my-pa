...
emits what it emitted on the day it merged, with one hundred and sixty-four capabilities and
...
CAPABILITIES_ADDED_AFTER_THE_CAPTURE_REVISION: Final[frozenset[str]] = frozenset(
    {
        ...
        "constraints.read",
        "constraints.search",
        # PC-CM-IMP-WP07. `f7a2c9d51e64` is the forward `ALTER` that admits the
        # twelve below. They are listed here for the same reason the six reads
        # above are: this set is the measured difference between the capture
        # revision's stored vocabulary and head's, and a member added to the
        # domain without a forward `ALTER` is caught by exactly that difference.
        ...
        "tasks.read",
        "tasks.search",
        # WP-TM-04/05. Later task-management revisions admit the write plane and
        # the commitment plane; they belong in this delta because the live
    ...
    try:
        ...
        assert set(triggers) == {
            ...
            *CONTEXT_TRIGGERS,
            ...
            "goodnotes_semantic_promotion_receipts_are_immutable",
            "project_constraint_revisions_are_immutable",
