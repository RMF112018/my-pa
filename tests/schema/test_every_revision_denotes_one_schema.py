...
**A defect measured on another branch is why this exists, and it is stated as
history rather than as a claim about this chain.** A first draft of
`9d4e7a3b1c62` restored `status IN ('extracted', 'quarantined', 'unsupported')`
in its `downgrade` while `8b3f5c17d904` — which builds `extractions` from the
*live* declaration — had been narrowed to two, so a database that went up and
back down to `7f2a9d6c4e18` admitted `quarantined` and a freshly built one at the
same revision refused it. Neither half is in this chain at the revision that adds
this file; WP-03 lands both afterwards, and this guard is deliberately installed
and measured *first*, against the chain as it stands, so that what it says about
the revisions added after it is a result rather than an expectation.

**Nothing here names a revision, a table, or a vocabulary in an assertion.** The
chain is walked out of `ScriptDirectory` and every fact is read out of the
catalogue, so the module is chain-agnostic: adding a revision extends what it
checks without editing it, and that is the property that makes it usable as an
acceptance gate for a package that adds revisions.
