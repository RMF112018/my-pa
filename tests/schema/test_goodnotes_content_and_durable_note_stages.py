...
def _frozen_names(path: Path, constant: str) -> list[str]:
    ...
    raise AssertionError(f"{path.name} does not declare {constant}")


def _declares(path: Path, constant: str) -> bool:
    return any(
        isinstance(node, ast.AnnAssign)
        and isinstance(node.target, ast.Name)
        and node.target.id == constant
...
def _latest_vocabulary_migration() -> Path:
    ...
    for entry in script.walk_revisions():
        path = Path(entry.path)
        if _declares(path, _ADMITTING_CONSTANT):
            return path
    raise AssertionError(
...
def test_the_chain_has_one_head_and_this_revision_is_on_it() -> None:
    ...
    assert len(list((ROOT / "migrations" / "versions").glob("*.py"))) == 107
