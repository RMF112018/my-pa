...
PREDECESSOR_DATABASE: Final = "my_pa_ri_predecessor_path_test"
FRESH_DATABASE: Final = "my_pa_ri_fresh_head_path_test"
...
@pytest.fixture
def disposable(monkeypatch: pytest.MonkeyPatch) -> Iterator[BuildDatabase]:
    ...
    def administer(*statements: str) -> None:
        with maintenance.connect().execution_options(isolation_level="AUTOCOMMIT") as connection:
            for statement in statements:
                connection.execute(text(statement))

    def build(name: str, revision: str) -> Disposable:
        administer(f'DROP DATABASE IF EXISTS "{name}" WITH (FORCE)', f'CREATE DATABASE "{name}"')
    ...
    try:
        ...
        administer(*(f'DROP DATABASE IF EXISTS "{one.name}" WITH (FORCE)' for one in built))
        maintenance.dispose()
