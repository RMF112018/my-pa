...
def test_the_process_acts_as_one_authenticated_local_operator(runtime: GatewayRuntime) -> None:
    ...
    transport that cannot reach one of the one hundred and two capabilities.
    """
...
def test_gsqs_b0_ports_are_empty_and_do_not_force_in_process_wait(
    runtime: GatewayRuntime,
) -> None:
    """Production GSQS is ChatLLM-driven; NAS does not post to RouteLLM."""
    from my_pa.application.goodnotes_gsqs_b0_workflow import WorkflowPorts, gsqs_b0_wait_in_process

    ports = runtime.service._gsqs_b0_ports
    assert ports is not None
    assert ports == WorkflowPorts()
    assert ports.poster is None
