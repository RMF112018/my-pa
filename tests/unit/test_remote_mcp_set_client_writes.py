...
def test_an_enabled_client_toggles_only_writes_enabled() -> None:
    with _repository() as (connection, repository):
        before = connection.execute(select(*remote_clients.c)).one()
        assert repository.set_client_writes(oauth_client_id=CLIENT_ID, writes_enabled=True)
        ...
        assert after.redirect_uris == before.redirect_uris
        assert after.registered_scopes == before.registered_scopes
...
def test_a_revoked_client_is_refused_and_unchanged() -> None:
    with _repository() as (connection, repository):
        ...
        assert not repository.set_client_writes(oauth_client_id=CLIENT_ID, writes_enabled=True)
...
def test_an_unknown_client_is_refused() -> None:
    with _repository() as (_, repository):
        assert not repository.set_client_writes(
            oauth_client_id="missing-client", writes_enabled=True
        )
