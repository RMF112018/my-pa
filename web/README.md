...
| `GET /api/project-controls/portfolio/constraints` | `constraints.portfolio_list`, `constraints.portfolio_search` | The Register across every Project the Principal owns, in one gateway call; a non-empty `q` selects search and applies that command's narrower allowlist. No Project identifier is admitted in the path, the query or anywhere else — the Project set is derived server-side from the session Principal, so the route cannot be used to ask whether a named Project exists. A page can stop for the ordinary page size or because the owned-Project cap was reached; the backend discloses which, and only the first is continuable |
| `GET /api/project-controls/portfolio/constraints/overview` | `constraints.portfolio_overview` | One entry per owned Project, each on that Project's own calendar and each the same shape the single-Project overview returns. No query, no Project identifier, and deliberately no portfolio-wide roll-up: counts taken against different Project calendars are not summable |
