...
export function Workbench({ initialState = DEFAULT_STATE }: { initialState?: WorkUrlState }) {
  ...
  function select(next: WorkView) {
    setState("loading"); setRows([]); setView(next); setQueryDraft(""); setCommittedQuery(""); setCursor(""); setSelectedTaskIds([]);
...
function CommitmentRows({ rows, waitingOn }: { rows: readonly CommitmentRow[] | readonly WaitingOnRow[]; waitingOn: boolean }) {
  return <>{rows.map((row) => {
    ...
    return <li key={row.commitment_id}><Link className="block rounded-xl border border-moss-slate/15 bg-surface p-4 hover:border-moss-green focus-visible:outline focus-visible:outline-2" href={`/work/commitments/${encodeURIComponent(row.commitment_id)}`}><span className="font-medium text-moss-slate">{row.title}</span><span className="mt-1 block text-sm text-muted">{row.counterparty?.display_name ?? "Counterparty not resolved"} · {waiting ? "waiting on · " : commitment ? `${commitment.direction.replaceAll("_", " ")} · ` : ""}{row.state}{row.due_date ? ` · due ${new Date(row.due_date).toLocaleDateString()}` : ""}</span>{waiting ? <span className="mt-1 block text-sm text-muted">Follow-up: {waiting.follow_up_task_title ?? "No linked follow-up Task"}{waiting.follow_up_task_state ? ` · ${waiting.follow_up_task_state.replaceAll("_", " ")}` : ""}</span> : null}</Link></li>;
  })}</>;
}
