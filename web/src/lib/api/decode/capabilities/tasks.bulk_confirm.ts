import { ok } from "../primitives";
import type { Decoder } from "../types";
import {
  decodeItems,
  pick,
  requiredBoolean,
  requiredIntGe,
  requiredString,
} from "./_mutation-helpers";

export interface TasksBulkConfirmResult {
  readonly bulk_operation_id: string;
  readonly affected: number;
  readonly no_op: number;
  readonly rejected: number;
  readonly history_ids: readonly string[];
  readonly replayed: boolean;
}

const KEYS = [
  "bulk_operation_id",
  "affected",
  "no_op",
  "rejected",
  "history_ids",
  "replayed",
] as const;

export const decodeTasksBulkConfirm: Decoder<TasksBulkConfirmResult> = (input) => {
  const known = pick(input, KEYS);
  if (!known.ok) return known;
  const id = requiredString(known.value.bulk_operation_id);
  if (!id.ok) return id;
  const affected = requiredIntGe(known.value.affected, 0);
  if (!affected.ok) return affected;
  const noOp = requiredIntGe(known.value.no_op, 0);
  if (!noOp.ok) return noOp;
  const rejected = requiredIntGe(known.value.rejected, 0);
  if (!rejected.ok) return rejected;
  const historyIds = decodeItems(known.value.history_ids, (item) => requiredString(item));
  if (!historyIds.ok) return historyIds;
  const replayed = requiredBoolean(known.value.replayed);
  if (!replayed.ok) return replayed;
  return ok({
    bulk_operation_id: id.value,
    affected: affected.value,
    no_op: noOp.value,
    rejected: rejected.value,
    history_ids: historyIds.value,
    replayed: replayed.value,
  });
};
